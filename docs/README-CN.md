...
**🤖 五层纵深防护，不靠模型自律**
L1 Prompt（LiteLLM pre/post + Bedrock Guardrail 双向）、L2 工具（`before_tool_call` ACL，default-deny）、L3 身份（Cognito JWT + RBAC + 东西向 iptables DROP）、L4 网络（IMDS 封锁 + DNS Firewall + NAT 出网白名单）、L5 凭据·只读·监控（guest 零长期凭据 + 只读黄金镜像盘 + Wazuh/auditd 运行时监控）。每层都落到代码或设备上。
...
**🚀 一行命令 CDK 部署**
`./setup.sh <region> <profile>` 几分钟拉起完整栈 — VPC、ASG、ALB、Lambda、DynamoDB、CloudFront、Cognito（authorization-code + PKCE）全部接通即用。云端 rootfs build 意味着**无需本地 Linux**（macOS / Windows 直接用）。
...
外部 SaaS 平台的用户经其自有 IdP 联邦进本平台单个 Cognito User Pool，Pre-Token-Generation Lambda 在签发 id_token 前注入 `custom:tenant_user_id` / `custom:platform_id`；授权判定始终在 hub 端查服务端账本（`owner_id`/`authorized_users`），不信客户端自报。

<p align="center">
  <img src="arch-federation-cn.png" alt="外部平台联邦身份接入端到端时序 — Cognito 联邦 + Pre-Token-Gen claim 注入" width="96%"/>
</p>
...
│  控制面（CRUD / 生命周期，SSM）                                        │
│                                                                       │
│  管理员/用户 ── API Gateway (HTTPS, x-api-key + Cognito + RBAC) ──┐   │
│                                                                   ▼   │
│                                                  Lambda ──► DynamoDB   │
│                                                    │        ├ tenants  │
│                                                    │        └ hosts    │
