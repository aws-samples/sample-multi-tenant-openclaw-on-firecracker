...
def effective_lt_version(profile, region, asg_name: str) -> tuple[str, list[str]]:
...
def managed_files(gateway: pathlib.Path) -> tuple[list[dict], list[str]]:
    ...
    unresolved: list[str] = []
    for m in CP_LINE.finditer(_expand_loops(text)):
...
def compare(files, s3_by_key, probes) -> list[dict]:
    rows = []
    for f in files:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        latest, findings = effective_lt_version(args.profile, args.region, args.host_asg)
        findings += unresolved
        ...
        print(f"\nverdict={'DRIFT' if drift else 'CONSISTENT'} "
              f"identical={len(rows) - len(drift)} drift={len(drift)} "
