...
**`POST {BASE}/hosts/{instance_id}/taint`** — 给宿主打 cordon 污点(**api-key 或 RBAC operator+**;持 API key 的运维脚本是主要调用方,故该路由跳过前置角色门、由处理函数内的身份门判定,viewer 与未验证 token 一律 403,**platform-scoped API key 也一律 403** —— 按 #108 它只能碰自己 platform 的租户,而污点作用于全局宿主)。#539 起,`is_tainted` 与生命周期 `status` **正交**,只表达 NoSchedule 运维意图:不动运行中的实例与其上租户,只标记该宿主让调度器在放置时避开它(排除逻辑见读侧 #540)。body `{reason:"<非空原因>"}`。幂等:已污点时返 `200 {..., already_tainted:true}` 且不覆盖原 `tainted_at/reason`。返 `200 {instance_id,is_tainted:true,tainted_at,tainted_reason}`;`reason` 缺失/为空返 `400 VALIDATION`,宿主不存在返 `404`(不会创建幽灵行)。审计字段 `tainted_by` 记录调用方身份,但不出现在 `GET /hosts` 列表里。`tainted_at` 是 epoch 秒的 **JSON number**,首次与幂等两条路径类型一致(#546 修复了幂等路径因 DDB `Decimal` 经 `default=str` 序列化成字符串的漂移;强类型客户端可安全按 number 解析)。
**`DELETE {BASE}/hosts/{instance_id}/taint`** — 清除污点(**api-key 或 RBAC operator+**,同上)。只 REMOVE 污点属性,**绝不持久化** `is_tainted:false`;`GET /hosts` 的 `is_tainted` 由响应层显式补 `false`。幂等:未污点时返 `200 {..., already_untainted:true}`。返 `200 {instance_id,is_tainted:false}`。

> **作用(#540 读侧已合入 bb,污点现已影响放置)。** 标记后调度器把该宿主从**所有**放置路径排除:同步 create(201)/restore/`process_pending`、队列 create(202,**生产默认路径**)、pinned create(`preferred_host_id`)→ `409`、migrate 显式 `target_host_id` → `409`(一律拒,不提供 force/allow_tainted 逃生口)。选点与认领之间的 TOCTOU 由 DDB 条件写兜底(`attribute_not_exists(is_tainted) OR is_tainted <> true`,脏值 fail-open 与读侧同口径);ASG 在途容量判据也排除污点机(避免"标完一批 1:6 机器、新机起不来、租户无处可迁")。**不驱逐、不停机、不迁移**该宿主上的存量租户;运维广播(`refresh-rootfs`/fleet action)与镜像操作(`pull-image`/`promote-canary`)仍照常覆盖污点机。
>
...
| `/hosts/{instance_id}/taint`                           | POST/DELETE    | api-key · operator           | 打/清 cordon 污点(NoSchedule,与 status 正交;platform-scoped key 403)                            |
