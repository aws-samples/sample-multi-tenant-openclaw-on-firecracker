...
# The balloon env vars only need to be set while host-agent.py is imported
# (it reads them into module constants at import). Clear them afterwards so the
# process-wide os.environ doesn't leak BALLOON_ENABLED=true into other test
# modules (e.g. test_migration imports handler.py and would otherwise see
# ballooning on and take the migrate 409 branch).
for _k in (
    "BALLOON_ENABLED",
...
class TestAdjustBalloons:
    ...
    def test_host_pressure_low_inflates(self):
        ...
        with (
            patch.object(agent, "_get_host_mem_info", return_value=(16384, 2000)),
            patch.object(
                agent,
                "_get_balloon_stats",
                return_value=_make_stats(available_mb=2048, actual_mib=0),
            ),
    ...
    def test_host_pressure_low_respects_min_guest(self):
        ...
        with (
            ...
            patch.object(
                agent,
                "_get_balloon_stats",
                return_value=_make_stats(available_mb=600, actual_mib=0),
            ),
            patch.object(agent, "_set_balloon_target") as mock_set,
        ):
            agent._adjust_balloons({"t1": {"vm_health": "up"}})
    ...
    def test_host_pressure_low_guest_already_tight(self):
        ...
        with (
            ...
            patch.object(
                agent,
                "_get_balloon_stats",
                return_value=_make_stats(available_mb=400, actual_mib=500),
            ),
            patch.object(agent, "_set_balloon_target") as mock_set,
        ):
            agent._adjust_balloons({"t1": {"vm_health": "up"}})
    ...
    def test_host_plenty_deflates(self):
        ...
        self._setup_vm("t1", 4096)
        with (
            patch.object(agent, "_get_host_mem_info", return_value=(16384, 8000)),
            patch.object(
                agent, "_get_balloon_stats", return_value=_make_stats(actual_mib=1000)
            ),
            patch.object(agent, "_set_balloon_target") as mock_set,
        ):
            agent._adjust_balloons({"t1": {"vm_health": "up"}})
    ...
    def test_host_plenty_already_zero(self):
        ...
        with (
            patch.object(agent, "_get_host_mem_info", return_value=(16384, 8000)),
            patch.object(
                agent, "_get_balloon_stats", return_value=_make_stats(actual_mib=0)
            ),
            patch.object(agent, "_set_balloon_target") as mock_set,
        ):
            agent._adjust_balloons({"t1": {"vm_health": "up"}})
    ...
    def test_host_moderate_no_action(self):
        ...
        self._setup_vm("t1", 4096)
        with (
            patch.object(agent, "_get_host_mem_info", return_value=(16384, 5000)),
    ...
    def test_max_inflate_ratio_cap(self):
        ...
        with (
            patch.object(agent, "_get_host_mem_info", return_value=(16384, 1000)),
            patch.object(
                agent,
                "_get_balloon_stats",
                return_value=_make_stats(available_mb=3500, actual_mib=0),
            ),
            patch.object(agent, "_set_balloon_target") as mock_set,
        ):
            agent._adjust_balloons({"t1": {"vm_health": "up"}})
...
class TestGetHostMemInfo:
    ...
    def test_reads_meminfo(self):
        ...
        with patch(
            "builtins.open", return_value=__import__("io").StringIO(mock_content)
        ):
