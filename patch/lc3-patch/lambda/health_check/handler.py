...
_STUCK_AND_TOKEN_REMOVES = (
    ...
    # 刹车与本文件补偿矩阵的触发标记。残留本身对下一轮无害(刹车用 deadline_enforced_for
    # 轮次锚,只认 for == 本轮死线;Codex 实现评审 #1 后受理路径不再 REMOVE ——
    # 无条件清会误清并发 suspend 的补偿标记),清掉是让行状态与"已归位"事实一致。
    # #679(Codex 实现评审 #4)—— 归位到终局时一并清生命周期租约:原操作已被刹车/静默期
    # 证明停手,租约再留着只会把「回正后立刻重发 suspend」「suspend_failed 后 delete」
    # 挡到 1800s 自然过期 —— 客户端拿到的"可行动状态"就不可行动了。旧 stuck 路径的
    # rollback 本来就等租约过期后才跑,顺带清过期字段无害。
    "restore_guest_ip, restore_host_port, deadline_enforced_at, "
    "deadline_enforced_for, active_lifecycle_op_id, active_lifecycle_action, "
    "active_lifecycle_until, lifecycle_lease_updated_at"
)
...
def _settle_enforced_suspend(t, now):
    ...
    """
    tid = t.get("id")
    # #679(Codex 实现评审 #3)—— 动作前**强一致重读**并核对:候选来自拍初的 scan 快照,
    # 到本行被处理时可能已隔几十秒 —— 期间执行者可能恰好完成迟到收尾(suspended)、客户端
    # 甚至可能已发起新一轮 restore(同 tid 的 VM 目录可能在同一台 host 上重生)。凭陈旧
    # 快照发 stop/rm 会删掉新生的 VM。重读后不符 → 撒手(行已被合法推进)。
    # 重读到发令之间仍有秒级窗口,由「新一轮要先经 suspended/restoring 状态翻转 + 归位
    # 条件写带死线锚」双重兜底;完全消除需要 host 侧世代锚,超出本轮(记录于 MR)。
    # 「两个 reaper invocation 并发,各自重读都过、一个 finalize 后另一个才发 rm」
    # (Codex 实现评审第二轮 #4)在本部署形态下不存在:health_fn 是
    # reserved_concurrent_executions=1(lambdas.py:1974)+ 单一 EventBridge rate 调度,
    # 同一时刻至多一个 invocation 在跑,重叠触发被 throttle 而不是并行执行。
    # 若未来放开并发,需先补 per-tenant 补偿 claim(见 MR 记录),不能只靠这次重读。
    try:
        _fresh = tenants_table.get_item(
            Key={"id": tid}, ConsistentRead=True
        ).get("Item")
    except Exception as e:  # noqa: BLE001 — 读不到就不动,下拍再来
        print(f"#679 settle {tid}: fresh read failed, hold: {e}")
        return "hold"
    if (
        not _fresh
        or _fresh.get("status") != "suspending"
        or _fresh.get("suspend_deadline") != t.get("suspend_deadline")
        or _fresh.get("deadline_enforced_at") != t.get("deadline_enforced_at")
    ):
        print(f"#679 settle {tid}: row moved on since scan — hold")
        return "hold"
    t = _fresh
        ...
        try:
            tenants_table.update_item(
                ...
                UpdateExpression=(
                    ...
                    "deadline_enforced_at, deadline_enforced_for, "
                    # 终局必须放租约(Codex 实现评审 #4):不放的话 suspend_failed 后唯一
                    # 合法的 delete 会被 LIFECYCLE_IN_FLIGHT 挡到 1800s 过期。
                    "active_lifecycle_op_id, active_lifecycle_action, "
                    "active_lifecycle_until, lifecycle_lease_updated_at, "
                    "vm_health, app_health, last_health_check"
                ),
...
def _enforced_restore_verdict(probe_ok, probe, host_dead, has_token):
    """#679 MR2 —— restore 补偿档的判定矩阵(纯函数,单测穷举;抄 suspend 档的形态)。

    restore **没有失败终态**:终局只有 running(迟到成功)与 suspended(打回,数据在
    S3 备份里原封未动)。与 suspend 档的根本差异:suspend 打回 suspended 会谎报数据
    新鲜度(备份→host 死窗口无上界),而 restore 打回 suspended **就是**数据的真实
    位置 —— 备份从未被消费成功,打回不丢任何东西。

    | 判据 | 动作 | 含义 |
    | 令牌缺 | revert | 预留从未落库(死在预留前)或已被释放(死在释放与打回之间)——
      两种都无残留,直接打回 |
    | 令牌在 ∧ host 亡(EC2 权威) | release_revert | VM 不可能活着,释放预留(host 行
      已消失 → 事务 host 项 CCF 归类 ALREADY,幂等)后打回 |
    | 令牌在 ∧ 探测不可达 | hold | 确认不了就不动(与 suspend 档同一条纪律) |
    | 令牌在 ∧ vm_dir ∧ fc_alive | promote | 恢复真的成了,凭令牌转正(既有
      `_promote_stuck_restore`,与原 Lambda 收尾同一步) |
    | 令牌在 ∧ fc_alive ∧ ¬vm_dir | mark | 矛盾:进程在、盘没了 —— 谁都不该信,标记人工 |
    | 令牌在 ∧ ¬fc_alive | stop_release_revert | launch 没成(盘可能恢复了一半)。照
      api 侧 #659 清场的 #520 C2 纪律:释放前必须 stop 确认(幂等 no-op),防"释放让
      物理号可复用而半拉子 VM 还活着"的串号方向 |
    """
    if not has_token:
        return "revert", "no reservation token — nothing reserved, flip back"
    if host_dead:
        return "release_revert", "restore target host is dead (EC2-confirmed)"
    if not probe_ok:
        return "hold", "restore target host unreachable — cannot confirm, no action"
    vm_dir = bool(probe.get("vm_dir"))
    fc_alive = bool(probe.get("fc_alive"))
    if fc_alive and vm_dir:
        return "promote", "VM is up on the restore target — late success"
    if fc_alive and not vm_dir:
        return "mark", "contradiction: firecracker alive but vm_dir missing"
    return (
        "stop_release_revert",
        "no live VM on the restore target — stop-confirm, release, flip back",
    )


def _release_stuck_restore_reservation(t, host_dead=False):
    """#679 MR2 —— 令牌化释放 restore 预留(reaper 侧)。

    **与 api 侧 `tenant_service._release_restore_reservation` 是同一步**(同一个事务
    形态:hosts 账本减量带下溢守卫 + tenants REMOVE 令牌与坐标、条件
    `restore_reservation_id = :rid`),不是第二个写手 —— 谁先消费令牌谁扣一次,
    后到的 ALREADY 幂等 no-op。跨包双份是刻意的(health_check 不能 import api),
    parity 测试钉住形态。

    **host_dead=True 时只动 tenant 项**(Codex MR2 实现评审 #1)—— 与 suspend 档
    原语 B 完全同构的教训:host 已亡时 hosts 行可能已被清掉/清零,双表事务的 host 项
    下溢守卫恒 CCF,而分类器对 host 项 CCF 判 RETRY(:1289,防真下溢),于是释放永久
    hold、租户永久卡 restoring。host 亡则账本行随之无意义,消费令牌 + 清坐标就是
    全部要做的事(物理槽位同理随 host 消失,不调 _release_phys_slot)。

    返回 True = 令牌已不在(本次消费或早已消费),可以安全打回;False = 瞬时失败,
    令牌可能仍在,本拍撒手(hold)。
    """
    tid = t.get("id")
    rid = t.get("restore_reservation_id")
    rhost = (t.get("restore_host_id") or "").strip()
    rvn = t.get("restore_vm_num")
    if not rid:
        return True
    if not rhost:
        # 令牌与坐标由同一个事务写入,按构造同在;缺坐标 = 未知写者,不猜(与 api 侧
        # fail-closed 同源)。留给 mark/人工。
        print(f"#679 settle {tid}: reservation token without coordinates — hold")
        return False
    if host_dead:
        try:
            tenants_table.update_item(
                Key={"id": tid},
                UpdateExpression=(
                    "REMOVE restore_reservation_id, restore_host_id, "
                    "restore_vm_num, restore_guest_ip, restore_host_port"
                ),
            ...
            if e.response.get("Error", {}).get("Code") == (
                ...
                # 令牌已被别人消费(原 Lambda 迟到释放)—— 目的已达成,幂等。
                return True
            print(f"#679 settle {tid}: dead-host token consume failed (hold): {e}")
            return False
        except Exception as e:  # noqa: BLE001 — 瞬时失败,下拍再来
            print(f"#679 settle {tid}: dead-host token consume failed (hold): {e}")
            return False
    try:
        hosts_table.meta.client.transact_write_items(TransactItems=[
            {"Update": {
                ...
                "ExpressionAttributeValues": {
                    ":v": int(t.get("vcpu", 0) or 0),
                    ":m": int(t.get("mem_mb", 0) or 0),
                    ":one": 1,
                },
            }},
            {"Update": {
        ...
        _rel = _classify_release_cancel(e, tid, "#679 restore-settle")
    if _rel == _REL_RETRY:
        return False
    # 占号单独还(自带 owner 条件,独立幂等单元;并进事务会被下溢守卫连带取消)。
    if rvn is not None:
        _release_phys_slot(rhost, rvn, tid)
    return True


def _settle_enforced_restore(t, now):
    """#679 MR2 —— restore 补偿档的一行。返回处置标签(调用方计数用),形态与
    `_settle_enforced_suspend` 逐字同源:强一致重读核对 → 判定 → 带死线锚的条件写。
    并发依据同 suspend 档(reserved_concurrent_executions=1,见彼处注释)。
    """
    tid = t.get("id")
    try:
        _fresh = tenants_table.get_item(
    ...
    if (
        not _fresh
        or _fresh.get("status") != "restoring"
        or _fresh.get("restore_deadline") != t.get("restore_deadline")
        or _fresh.get("deadline_enforced_at") != t.get("deadline_enforced_at")
    ):
        print(f"#679 settle {tid}: row moved on since scan — hold")
        return "hold"
    t = _fresh
    dl = t.get("restore_deadline")
    rhost = (t.get("restore_host_id") or "").strip()
    rid = t.get("restore_reservation_id")
    if rid and rhost:
        host_dead = _host_is_dead(rhost)
        if host_dead:
            probe_ok, probe = False, {}
        else:
            probe_ok, probe = _probe_host_tenant_state(rhost, tid)
    else:
        host_dead, probe_ok, probe = False, False, {}
    action, reason = _enforced_restore_verdict(
        probe_ok, probe, host_dead, has_token=bool(rid)
    )
    print(f"#679 settle {tid}: restore action={action} ({reason})")
    ...
    if action == "mark":
        ...
        return "mark"
    if action == "promote":
        return "settled" if _promote_stuck_restore(t, now) else "mark"
    if action == "stop_release_revert":
        # stop 确认(#520 C2:释放前必须停;stop-vm.sh 幂等,VM 不在即 no-op)。
        # 裸命令不带 host_guard:launch 前刹车 + 静默期已保证原执行者停手,唯一并发
        # 是下一拍的自己(幂等)。刻意不 rm 残盘 —— 与 api 侧 #659 清场同款取舍,
        # 下一次 launch 会覆盖,disk_gc 兜底。
        _q_tid = shlex.quote(str(tid))
        _q_vm = shlex.quote(str(int(t.get("restore_vm_num", 1) or 1)))
        ok, _out = _ssm_run_capture(
            rhost, f"/home/ubuntu/stop-vm.sh {_q_tid} {_q_vm}", timeout=30
        )
        if not ok:
            return "hold"
        action = "release_revert"
    if action == "release_revert":
        if not _release_stuck_restore_reservation(t, host_dead=host_dead):
            return "hold"
    # action ∈ {"revert", "release_revert"} —— 打回。目标**从哪来回哪去**(与 api 侧
    # 失败回滚的 `_rollback_to` 同一判据):`suspend_failed` 入口的 restore 打回
    # `suspend_failed` 原样 —— 打回 suspended 是语义提升(它的数据只到备份时刻,
    # owner 2026-08-29 裁定禁止洗白);其余(含 prev 缺失的存量行,它们只可能从
    # suspended 进入)打回 suspended。
    # REMOVE 清单**手写**(照原语 B 的做法),刻意不含 `suspend_backup_key`:
    # suspend_failed 入口打回后客户端 restore 的准入与取数全靠它(no-data-loss);
    # 真 suspended 入口行上本就没有该字段,不清也是 no-op。restore_backup_key 同理
    # 不清 —— 它是 suspended 态的数据指针。restore 令牌/坐标此刻已按构造不在
    # (revert 格从未写入、release 格刚被事务消费),REMOVE 是 no-op 兜底。
    _flip_to = (
        "suspend_failed"
        if t.get("lifecycle_prev_status") == "suspend_failed"
        else "suspended"
    )
    try:
        tenants_table.update_item(
            Key={"id": tid},
            UpdateExpression=(
                "SET #s = :flipto, updated_at = :t, lifecycle_stuck_reason = :r "
                "REMOVE lifecycle_prev_status, restore_guest_ip, restore_host_port, "
                "restore_reservation_id, restore_host_id, restore_vm_num, "
                "lifecycle_stuck_at, lifecycle_stuck_vm_dir, "
                "lifecycle_stuck_fc_alive, updated_at_stuck_seen, "
                "lifecycle_rollback_deferred_until, lifecycle_probe_attempted_at, "
                "suspend_release_id, deadline_enforced_at, deadline_enforced_for, "
                "active_lifecycle_op_id, active_lifecycle_action, "
                "active_lifecycle_until, lifecycle_lease_updated_at"
            ),
            # `attribute_not_exists(restore_reservation_id)` 是第二道防线(Codex MR2
            # 实现评审 #2):强一致重读到本写之间,执行者理论上可完成一次新预留(预留在
            # launch 前刹车【之前】,判死后仍合法)。第一道防线是静默期时序(预留事务的
            # SDK 重试最坏 ~71s < 150s,reaper 动手时预留必已落库、走 release 档);这道
            # 锚把时序论证变成机械保证 —— 令牌若在,打回会把新令牌清成无主账本泄漏。
            ConditionExpression=(
                "#s = :restoring AND restore_deadline = :dl "
                "AND attribute_not_exists(restore_reservation_id)"
            ),
            ExpressionAttributeNames={"#s": "status"},
            ExpressionAttributeValues={
                ":flipto": _flip_to,
                ...
                ":dl": dl,
                ":t": now.isoformat(),
                ":r": f"#679: flipped back to {_flip_to} — {reason}",
            },
        )
        _emit_audit(
            "tenant.lifecycle_converged",
            {"tenant_id": tid, "action": "restore_flip_back", "host_id": rhost},
        )
        return "settled"
    except ClientError as e:
        if e.response.get("Error", {}).get("Code") == "ConditionalCheckFailedException":
            print(f"#679 settle {tid}: flip-back raced — row moved on")
            return "hold"
        raise
...
def _reap_stuck_lifecycle(now):
    ...
    for t in stuck:
        tid = t.get("id")
        status = t.get("status")
        # ── #679 —— 围栏判死过的 suspending/restoring 行走补偿矩阵,**不走**旧 stuck 路径 ──
        ...
        # 显式刹车停手(suspend 在 stop-vm 前、restore 在 launch-vm 前,MR2 起两档齐)。
        _enforced = t.get("deadline_enforced_at")
        if _enforced is not None and status in ("suspending", "restoring"):
            try:
                _quiet_left = (
            ...
            _enforced_done += 1
            _out = (
                _settle_enforced_suspend(t, now)
                if status == "suspending"
                else _settle_enforced_restore(t, now)
            )
            if _out == "settled":
                rolled_back += 1
            elif _out == "mark":
                ...
                unconfirmed += 1
            if _out != "settled":
                # #679(Codex 实现评审第二轮)—— hold/mark 的行必须推进轮转键。候选集
                # 按 lifecycle_probe_attempted_at 升序排(:1985),而补偿矩阵限额只有
                # _ENFORCED_MAX_PER_SWEEP=2:不推进,则同一批探不通/矛盾的行每拍都排
                # 最前、吃光两个名额,后面的 enforced 行【永远】轮不到 —— 与旧路径
                # codex 第十五轮修过的饿死同型。settled 不需要:行已翻走,离开候选集。
                # best-effort + 状态锚,与旧路径那处(:2075)同款。
                try:
                    tenants_table.update_item(
                        Key={"id": tid},
                    ...
                    print(f"#679 settle {tid}: probe-stamp best-effort failed: {e}")
            continue
