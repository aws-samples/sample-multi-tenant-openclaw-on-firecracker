...
// (对齐主流交易所 AI 助手的 StatusParts 推理展示)。返回 interval id,调用方在
...
// Next-Actions: 每条回复后给 2-3 个相关「下一步」建议(对齐主流交易所 AI 助手
...
function renderMd(s) {
  ...
  const base = esc
    ...
    .replace(/\n/g, "<br>");
  // 行情卡片(对齐主流交易所 AI 助手的图表/widget 可视化):agent 回行情时是纯
...
async function send() {
  ...
  try {
    ...
    // 思考过程面板(对齐主流交易所 AI 助手 StatusParts):agent 处理期间,根据用户
    ...
    // 占位)时,不当普通回复渲染,改成像主流交易所 AI 助手那样的明确安全提示。
