#!/usr/bin/env bash
# Install the API Gateway route added by patch 376 (POST /create-image-snapshot) without a
# CloudFormation deploy. Existing GET /images is the source of truth for method auth (key_required,
# operator+) and the Lambda AWS_PROXY alias integration — the new route reuses it verbatim.
set -euo pipefail

CONFIRM="APPLY"
STATE_DIR="${OC_APPLY_API_STATE_DIR:-$HOME/.oc-apply-api-routes-376}"
TARGET_PATHS=("/create-image-snapshot")
TARGET_PARTS=("create-image-snapshot")
TARGET_METHODS=("POST")
...
case "$CMD" in
  ...
  apply)
    ...
    _print_plan
    _assert_targets_absent
    _gate "create the patch-376 route POST /create-image-snapshot on API '$API' and deploy stage '$STAGE'"
    _preflight
    _assert_targets_absent
    ...
    for i in "${!TARGET_PATHS[@]}"; do
      ...
      parent="$ROOT_ID"  # patch 376: single root-level resource
    ...
    NEW_DEPLOY="$(_q apigateway create-deployment --rest-api-id "$API" \
      --description "patch 376 hot route" | jq -r .id)"
    ...
    _verify
    echo "OK: route deployed; rollback state is $ST"
    ;;
  verify)
    _preflight
    _verify
  ...
  rollback)
    ...
    # Two expected post-apply stage states, both safe to roll back:
    #   current == NEW_DEPLOY      -> apply flipped the stage; flip it back to PREVIOUS.
    #   current == PREVIOUS_DEPLOY -> apply crashed after saving new_deployment but before/at a
    #                                 failed update-stage; stage never moved, so skip the flip and
    #                                 just remove the orphan resources we created.
    # Any third value is a genuine newer deployment by someone else -> refuse to overwrite it.
    if [ -n "$NEW_DEPLOY" ]; then
      CURRENT_DEPLOY="$(_q apigateway get-stage --rest-api-id "$API" --stage-name "$STAGE" \
        | jq -r '.deploymentId // empty')"
      if [ "$CURRENT_DEPLOY" = "$NEW_DEPLOY" ]; then
        _q apigateway update-stage --rest-api-id "$API" --stage-name "$STAGE" \
          --patch-operations "op=replace,path=/deploymentId,value=$PREVIOUS_DEPLOY" >/dev/null
        echo "stage flipped $NEW_DEPLOY -> $PREVIOUS_DEPLOY"
      elif [ "$CURRENT_DEPLOY" = "$PREVIOUS_DEPLOY" ]; then
        echo "stage already at $PREVIOUS_DEPLOY (apply never flipped it); removing orphan resources only"
      else
        echo "FATAL: stage is at $CURRENT_DEPLOY, neither the apply deployment ($NEW_DEPLOY) nor the" >&2
        echo "       pre-apply deployment ($PREVIOUS_DEPLOY); a newer deployment exists — refusing to overwrite it." >&2
        exit 2
      fi
    fi
