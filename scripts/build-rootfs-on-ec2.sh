...
RUNNER_KEY="deployment/builder/runner-$$.sh"
"${AWS[@]}" s3 cp "$BUILD_SCRIPT" "s3://${BUCKET}/${RUNNER_KEY}" --quiet
rm -f "$BUILD_SCRIPT"

CMD_ID=$("${AWS[@]}" ssm send-command \
  --instance-ids "$INSTANCE_ID" \
  --document-name AWS-RunShellScript \
  --parameters "{\"commands\":[\"set -e; cloud-init status --wait || true; export DEBIAN_FRONTEND=noninteractive; apt-get update -qq && apt-get install -y -qq debootstrap pigz e2fsprogs curl unzip jq; if ! command -v aws >/dev/null; then curl -sL 'https://awscli.amazonaws.com/awscli-exe-linux-${AWSCLI_ARCH}.zip' -o /tmp/awscliv2.zip && cd /tmp && unzip -qo awscliv2.zip && ./aws/install >/dev/null; fi; aws s3 cp s3://${BUCKET}/${RUNNER_KEY} /tmp/runner.sh --region ${REGION} --quiet && bash /tmp/runner.sh\"]}" \
  --comment "openclaw rootfs build $VERSION ($ARCH)" \
  --timeout-seconds 1800 \
  --query 'Command.CommandId' --output text)
echo "  CommandId: $CMD_ID"

# Cleanup hook for the runner script.
trap '_cleanup_runner; _cleanup' EXIT
_cleanup_runner() {
  set +e
  "${AWS[@]}" s3 rm "s3://${BUCKET}/${RUNNER_KEY}" --quiet 2>/dev/null
}
