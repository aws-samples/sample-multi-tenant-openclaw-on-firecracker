...
共享 ssm client + VM_PORT_BASE 从 core.clients import;stdlib(base64/json/shlex/time)
本模块自带。按 design.md 层间契约:core 域不反向 import services/routes。
...
def _launch_vm_wake_cmd(tenant_id, item):
    ...
    apiKey / channel_secret / Cognito 唤醒路径的语义:
      ...
      • INJECTED_COGNITO_B64(第 11 位):同上,数据盘上的 Cognito 字段是首次注入的,
        wake 传空不动它。
