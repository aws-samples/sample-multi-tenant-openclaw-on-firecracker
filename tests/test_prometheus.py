...
def _synth_template(metrics_enabled=True, use_managed=True):
    ...
    NOTE (2026-06-30 架构决策): AMP/AMG 托管 workspace 只在 metrics.use_managed=true
    时才建；默认（仅 enabled=true）走自建 EC2 Prometheus+Grafana、不建任何托管资源
    （规避 AMG 强制 AWS_SSO）。这些断言托管 workspace 存在的测试因此须显式开 use_managed。
    """
    ...
    m["enabled"] = metrics_enabled
    m["use_managed"] = use_managed
...
class TestPromExporter:
    ...
    def test_write_ddb_mirrors_metrics_back_into_status_for_prom_exporter(self):
        ...
        assert (
            'info["metrics"] = metrics' in src or 'info["metrics"] = metrics' in src
        ), (
...
class TestADOTConfig:
    def test_init_host_installs_collector(self):
        ...
        assert "aws-otel-collector" in sh or "adot-collector" in sh, (
