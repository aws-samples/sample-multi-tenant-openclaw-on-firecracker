...
# 适配是针对特定版本(2026.5+ schema,见 CHANGELOG launch-vm schema break 条)。
# 升级 OpenClaw = 改这里的版本号 + 验证 launch-vm jq 改写仍匹配新 schema → 重烤。
OPENCLAW_PIN="2026.5.6"
...
if [ -d /tmp/image-sample ]; then
  ...
  if [ -d /tmp/image-sample/security ]; then
    echo "  → baking + enabling plugins (acl-guard + sentinel-guard + claw-channel)"
    ...
    # Register all THREE plugins in config: load paths + enabled entries. jq merge
    # keeps whatever the template already had under .plugins.
    #   acl-guard / sentinel-guard  — before_tool_call hooks (priority 1000 / 200)
    #   claw-channel               — external C-end channel (signed webhook). It
    #     replaces the bare /v1/chat/completions endpoint; the per-account HMAC
    #     secret is injected at LAUNCH (launch-vm.sh) into channels.claw-channel,
    #     NOT baked here — the read-only golden image carries zero credentials.
    if command -v jq >/dev/null 2>&1; then
      jq '
        (.plugins // {}) as $p |
        .plugins = ($p + {
          "load":    (((($p.load // {})) + { "paths": (((($p.load // {}).paths) // []) + ["/home/agent/.openclaw/plugins/acl-guard", "/home/agent/.openclaw/plugins/sentinel-guard", "/home/agent/.openclaw/plugins/claw-channel"] | unique) })),
          "entries": (((($p.entries // {})) + { "acl-guard": { "enabled": true }, "sentinel-guard": { "enabled": true, "hooks": { "allowConversationAccess": true } }, "claw-channel": { "enabled": true } }))
        })
        # claw-channel is a single-account channel: enable it under channels.<id>
        # so the gateway calls startAccount and the signed webhook comes up. The
        # HMAC secret is intentionally left empty here — launch-vm.sh injects a
        # per-VM secret at launch (zero credentials in the read-only image).
        | .channels = ((.channels // {}) + { "claw-channel": (((.channels // {})["claw-channel"] // {}) + { "enabled": true, "secret": "" }) })
