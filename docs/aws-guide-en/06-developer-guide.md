...
> ⚠️ **Note (superseded): the real-time chat path has moved to two-tier routing; the "chat plane / claw-hub" token-exchange contract in this chapter is obsolete**
>
> The real-time chat portions of this chapter were written against the earlier claw-hub WebSocket hub model (browser → `/hub/token` for a frontend token → `wss /hub/ws` → claw-channel). That model was replaced on 2026-07-08 by two-tier routing (platform backend WebSocket gateway → OpenResty edge → microVM gateway, with identity based on a platform session JWT + Ed25519 device authentication). **For real-time chat integration, refer to the "Data-plane two-tier routing" chapter (Chapter 13).** Everything below that mentions `claw-hub`, `claw-channel`, `/hub/token`, `/hub/ws`, `/channel-token`, or the chat frontend/channel HMAC tokens is a superseded contract, kept only to explain the evolution. **The control plane REST API and authorization model portions of this chapter remain valid.**

The solution exposes two integration surfaces: the **control plane REST API** (management operations such as tenant registration, lifecycle, backup, and deregistration, via `x-api-key` + optional Cognito RBAC, still valid) and the **real-time chat path** (now two-tier routing, see Chapter 13).
...
The solution has two orthogonal identity planes, each with its own independent trust root:

- **Control plane (REST API, still valid)**: the first gate is `x-api-key`; when RBAC is enabled, Amazon Cognito token verification is layered on top. Amazon Cognito is AWS's identity platform for web and mobile applications; here it issues id_tokens (RS256) for control-plane console sign-in, verified by the control-plane Lambda with Cognito's public keys (JWKS, JSON Web Key Set). A failed verification is uniformly downgraded to least privilege (read-only viewer), and a pure `x-api-key` request without a Bearer takes its role from `DEFAULT_NO_JWT_ROLE` (default `viewer`). The resource-owner (`owner_id`) check is enforced independently of the RBAC switch.
- **Data plane (real-time chat, two-tier routing)**: the first hop uses a platform self-signed session token (HS256 JWT, not Cognito); the second hop uses OpenClaw's native Ed25519 device asymmetric authentication plus a per-tenant gateway token. See Chapter 13.

> **Note**
>
> The three token types and the `/hub/*` endpoints described in the "chat plane / claw-hub" subsections below all belong to the superseded old data-plane model (see the warning box at the top of this chapter). For current data-plane identity, refer to Chapter 13.
...
The RBAC role levels are viewer (read-only) < operator < admin. Routes on the read-only allowlist are reachable by viewer; the rest require operator and above; the role check runs uniformly after a route is matched and before the business logic executes. **The owner check is decoupled from `RBAC_ENABLED` and always runs**: `RBAC_ENABLED` gates only the per-route _role_ check (viewer/operator/admin) and no longer doubles as the master switch for the owner check. Any non-admin caller can access only resources under its own `owner_id` for every per-tenant record, regardless of the `RBAC_ENABLED` value — turning RBAC off does not turn `GET/PUT/DELETE /tenants/{id}` into a cross-tenant IDOR. With RBAC off, every caller resolves to the API-key admin identity, so single-tenant control-plane semantics hold naturally. `RBAC_ENABLED` defaults to `true`; disable it explicitly only when a demo or single-tenant development environment needs the _role_ gate open. Confirm the target environment's value before integrating.
...
**RBAC: viewer.** Lists tenants; a non-admin sees only tenants of their own `owner_id` (the owner filter always applies, decoupled from `RBAC_ENABLED` — admin/API-key callers see everything). Supports `?tag=key:value` multi-value filtering (AND semantics).
...
- **GET /audit-log** (RBAC viewer): lists audits (newest-first), with query parameters `limit` (default 50, clamped to `[1,500]`) and `since` (ISO-8601); returns 200 `[]` without an audit_table. Audit best-effort records all POST/PUT/DELETE; the Amazon DynamoDB table has a 90-day TTL after which entries are removed — this is a mutable audit trail, not immutable compliance evidence. A WORM-grade immutable audit trail is planned (not implemented, see issue #32). **Isolated by caller identity**: admin and API-key callers see everything; ordinary users (viewer/operator) see only operations they initiated (server-side filtered by `actor_owner_id`), and a token that fails verification returns 403, preventing unauthorized enumeration of other tenants' audit trails. The owner filter is independent of the RBAC master switch.
...
| `chatCompletions` endpoint                                                                                                                                 | Integrators wanting an external chat HTTP endpoint                                                                                                                                                                                                        | Explicitly deleted by default (injected only for tenants that pass `chat_endpoint_enabled:true` at registration) | The per-tenant switch is implemented (`tenant_service.py:824/1040` + `core/ssm_dispatch.py:48/118`); enable per tenant, not globally                            |
