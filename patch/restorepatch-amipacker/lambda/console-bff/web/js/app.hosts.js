...
window.ocHosts = {
  ...
  async pullSnapshotToAll() {
    ...
    for (const h of targets) {
      ...
      try { await this.api('POST', `hosts/${h.instance_id}/pull-image?snapshot_time=${encodeURIComponent(st)}&slot=live`); ok++; }
      catch { fail++; }
    }
  ...
  async pullSnapshotToHost(host_id) {
    ...
    try {
      const r = await this.api('POST', `hosts/${host_id}/pull-image?snapshot_time=${encodeURIComponent(st)}&slot=live`);
      this.toast = `✓ live pull started (job ${r.job_id ? r.job_id.slice(0, 10) : '?'})`;
    } catch (e) {
