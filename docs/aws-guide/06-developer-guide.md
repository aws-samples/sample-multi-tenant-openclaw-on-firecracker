...
本节介绍该解决方案的认证模型，包括唯一信任根、三种 token、两个验签平面、基于角色的访问控制（RBAC，Role-Based Access Control）三级角色，以及零凭据约束。

### 信任根与验签平面

该解决方案的身份验证核心是 Amazon Cognito，它是整个平台的唯一信任根。Amazon Cognito 是 AWS 面向 Web 和移动应用的身份平台，提供用户目录、认证服务器和 OAuth 2.0 授权；该解决方案用它为租户和终端用户签发 id_token（JSON Web Token，JWT），由各组件用 Cognito 的公钥（JWKS，JSON Web Key Set）校验。

系统中出现的三种 token、两个验签平面，信任全部回溯到由 Amazon Cognito 签发并由其公钥验签的 id_token。控制面验它，数据面 `claw-hub` 也只验它；外部 OpenID Connect（OIDC）联合登录的用户也是联合进同一个 Cognito 用户池（User Pool）后由 Cognito 重新签发 id_token。该平台自身不维护独立的用户密码体系，也不信任任何客户端断言的身份，一切以 Cognito 验签得到的用户唯一标识（`sub`）为准。验签失败的请求一律降级为最小权限（只读 viewer）处理，不放行为高权限。
...
> **Note**
>
> 本节描述的 hub 进程内裸路径 `/token`、`/channel-token`、`/ws`、`/files/*`，对外经 Amazon CloudFront `/hub/*` 前缀暴露为 `/hub/token`、`/hub/ws` 等。下文小节用裸路径描述 hub 契约，端到端接入序列用对外的 `/hub/*` 路径，两者指同一端点。

### 三种 token 概览

下表汇总该解决方案中三种 token 的发行方、验签方、载体与生存时间（TTL，Time To Live）。
...
> **Important**
>
> 不带 Bearer token 的请求被当作可信自动化处理，此时调用者身份是内置的「密钥调用者」（api-key），等同 admin 全权限。也就是说网络层面已可信的内部自动化（无 JWT）拿到的是全权限。对外暴露该 API 时，必须保证「无 Bearer 即全权」这条路径只对可信网络开放（经 Amazon API Gateway，带 `x-api-key`）。Amazon API Gateway 是 AWS 托管的 API 网关，承载 REST/HTTP API 并对接后端；该解决方案用它对外暴露控制面 API。

### RBAC 三级角色

RBAC 角色等级为 viewer（只读）< operator（运维）< admin（管理）。只读白名单内的路由 viewer 可达，其余需 operator 及以上；角色检查在路由命中后、业务逻辑执行前统一运行；owner 检查仅在 RBAC 开启时执行。`RBAC_ENABLED` 默认为 `true`，即默认强制按路由的角色检查与属主检查；仅在 demo 或开发环境需要完全开放时才显式关闭。接入前请确认目标环境的取值。
...
**RBAC：viewer。** 列所有租户；`RBAC_ENABLED` 时非管理员只看自己 `owner_id` 的租户。支持 `?tag=key:value` 多值过滤（AND 语义）。
...
- **GET /audit-log**（RBAC viewer）：列审计（newest-first），query 参数 `limit`（默认 50，max 500）、`since`（ISO-8601）；无 audit_table 返回 200 `[]`。审计 best-effort 记录所有 POST/PUT/DELETE，TTL 90 天。
...
本节介绍实时聊天链路的接入方式。链路采用 outbound-WebSocket 中枢模型：租户 microVM 内的 `claw-channel` 主动向 `claw-hub` 拨出连接，浏览器经 CloudFront 接入同一中枢，双方在中枢按租户撮合，microVM 不开放任何入站端口。

聊天链路是 outbound-WebSocket 中枢模型：租户 microVM 内的 `claw-channel` 主动拨出连 `claw-hub`，不开入站端口。浏览器和 agent 之间的实时投递全走 hub 的 WebSocket：浏览器 ↔ `claw-hub` ↔ 租户 microVM 内 `claw-channel` ↔ agent。

### WebSocket 帧结构

WebSocket wire 帧结构如下：
...
前端到通道路由时，hub 用服务端验证过的 `sub` 作 `senderId`（不信任客户端断言），转发到该租户的通道。

### 回复帧类型

回复帧分三类：

- **reply**：最终答复，`type:"reply"`。
- **reply_delta**：流式累积文本，`type:"reply_delta"`、`operationType:"msg_update"`。channel 每 250ms 发一个 delta 帧，hub 转给前端，前端按 `clientMessageId` 定位到该请求的气泡并 REPLACE（非 append），最终由标准 `reply` 帧定稿。
- **reply_error**：失败。

### 并发与多会话

每条消息客户端 stamp 一个 `clientMessageId`，hub 在每个 reply/reply_delta/error 帧上原样回显，前端按此 id 路由到精确的 pending 请求而非 FIFO。channel 内部用 per-session serial chain（`Map<sessionKey, Promise>`），每个 sessionKey 顺序处理、不同 session 并行。

客户端可选传 `threadId`（正则 `^[A-Za-z0-9._:-]+$`，max 80），hub 校验后转发，缺失回退到 `sub`（单线程向后兼容）。会话隔离维度上 channel 强制 `dmScope="per-channel-peer"`，`peerId = threadId ? "{cognitoSub}:t:{threadId}" : cognitoSub`，每个 sub 加 threadId 拿独立会话。含注入字符的恶意 threadId 被正则清洗丢弃回退 `sub`，超长（>80）被截断。

### 出图与看图链路

媒体（media）链路分看图（inbound）与出图（outbound）两条：

- **看图（inbound）**：用户上传 → 前端经 hub presign PUT 到 Amazon S3 → 发 FILE 帧含 `fileKey` → hub 转发 → channel 调 `/files/download-url` 取预签名 GET → 本地校验 MIME 与 size 后写临时文件 → 传给 core multimodal。
- **出图（outbound）**：agent 引用本地文件 → channel 校验 MIME 与 size 并经 hub presign 上传 Amazon S3 → `fileKey` → 发 FILE 帧 → hub 转发 → 前端调 `/files/download-url` 取预签名 URL。

图片保护采用 MIME 白名单（image/png、image/jpeg、image/gif、image/webp 等），单条消息最多 4 张；身份文件（SOUL.md、AGENTS.md、IDENTITY.md 等）禁止通过 outbound media 上传。

> **Note**
>
> 上传大小上限（20MB）是软约束：PUT presign 不能用 ContentLengthRange，靠 ContentType pin 加申报 size 软校验，不是 Amazon S3 硬强制。

### 文件端点与 IDOR 防护

`/files/upload-url` 和 `/files/download-url` 都校验 Bearer 头或 `?token=` 里的 hub token（前端或通道 token 都接受）。文件在 Amazon S3 的存放路径按租户分段，取下载链接时校验路径里的租户段必须与 token 里的租户一致（防越权访问别人的文件，跨租户连「存不存在」都不泄露）。前端发文件帧时 hub 也会校验文件归属等于该前端的租户，否则静默丢弃。Amazon S3 媒体桶的跨域（CORS）只放行单一 Amazon CloudFront 域，方法限 GET/PUT，不开 `*`。

Amazon CloudFront 是 AWS 的全球内容分发网络，在边缘节点缓存与加速；该解决方案用它做公网唯一入口（Amazon CloudFront → ALB），反向代理聊天 UI 和 hub。Amazon CloudFront 的 `/hub/*` 行为配置为 `origin_request_policy=ALL_VIEWER`（转发所有请求头含 Upgrade 与 Sec-WebSocket-\*）、`cache_policy=CACHING_DISABLED`、`allowed_methods=ALLOW_ALL`、origin read_timeout=60s（对应 WebSocket 长连接）。

> **Important**
>
> 媒体对象的就绪检查目前只是占位：能取到对象即视为就绪，未接病毒或内容扫描。接入方不应对外宣称上传文件已做内容安全扫描；要扫描需自行在链路上添加（参见『能力边界』）。

### hub 端 HTTP 状态码速查表

下表汇总 hub 端（`/token`、`/channel-token`、`/files/*`、WebSocket）的返回码。控制面 REST 端点的状态码见各端点行内。

| 状态码  | 端点 / 触发条件                                                    |
| ------- | ------------------------------------------------------------------ |
| 401     | `/token`：无 `Bearer ` 前缀 / Cognito 验签失败或无 sub             |
| 400     | `/token`：body 无 `tenant_id`                                      |
| 403     | `/token`：租户授权检查不通过（非该租户 owner/授权/共享）           |
| 200     | `/token`：返回 `{token, expires_in:300}`                           |
| 401     | `/channel-token`：通道签名校验失败（签名/时间窗/秘密）             |
| 200     | `/channel-token`：返回通道 token                                   |
| 503     | `/files/*`：`ASSETS_BUCKET` 未配置                                 |
| 401     | `/files/*`：无有效 hub token                                       |
| 415     | `/files/upload-url`：mimeType 不在白名单                           |
| 413     | `/files/upload-url`：声明 size 超 `MEDIA_MAX_BYTES`                |
| 404     | `/files/download-url`：fileKey 形状错 / 跨租户（IDOR，不泄存在性） |
| 202     | `/files/download-url`：对象尚未就绪（HeadObject 失败）             |
| 500     | hub 各端点：未捕获异常兜底                                         |
| WS 1008 | hub WebSocket 握手：hub token 验签失败，发 error 帧后 close        |

> **Note**
>
> hub 端 media 端点的 200 响应体用 `{code:"000000", success:true, data}` 包装。

### 错误处理与超时

- 空消息（无文本且无图片）被 hub 拒绝，返回 `{type:"error", error:"empty message"}`。
- agent 离线（找不到该租户 channel 连接）返回 `{type:"error", error:"agent offline"}`。
- 坏 JSON 帧 hub 返回 `{type:"error", error:"bad json"}`，channel 侧 try-catch 忽略，连接不崩。
- 前端换 hub token（`POST /token`）返回 401 时，先用 refresh_token 静默换新 id_token 重试一次，换不动才清本地 token 跳 Hosted UI 重登。
- 前端对瞬态故障（Amazon CloudFront 抖动、WebSocket 重连窗口、token TTL 边界）自动退避重试，单次握手设 12s 总超时。
- channel 关键决策（token-fail、inbound-media-skip）写本地日志供审计。

### 端到端最小接入序列

以下编号步骤描述前端从登录到收发一条消息的最短路径，把认证与帧结构串起来（curl 示意，`{...}` 占位，凭据 `[REDACTED]`）。

1. **拿 Cognito id_token**：经 Cognito Hosted UI 的 authorization-code + PKCE（可经外部 OIDC 联合），用 `code` 加 `code_verifier` 调 `/oauth2/token` 换 id_token 加 refresh_token。
2. **换前端 hub token**：`POST /hub/token`（经 Amazon CloudFront `/hub/*` → ALB → hub），`Authorization: Bearer {id_token}`，body `{"tenant_id":"{tenant}"}`；hub 验 JWKS 加授权检查后返回 `{token, expires_in:300}`，授权失败 403：
...
3. **连 WebSocket**：`GET wss://<your-distribution>.cloudfront.net/hub/ws?token={hub token}`；成功收 `{type:"ready", tenant}`，验签失败收 error 帧加 close 1008。
4. **发消息**：在 WebSocket 上发一帧：`{messageId, clientMessageId:"{uuid}", senderType:"USER", parts:[{kind:"TEXT", text:"hi", isDone:true}], threadId?}`；`clientMessageId` 由客户端 stamp UUID 用于回包对账。
5. **收回复**：同一 WebSocket 按 `clientMessageId` 收 `reply_delta`（流式累积，REPLACE 当前气泡）和最终 `reply` 帧定稿，失败收 `reply_error`。hub token 临过期（前端 270s 阈值）主动重连换新，id_token 过期则 refresh_token 静默换新。

出图与看图另需 `/files/upload-url`（PUT presign）和 `/files/download-url`（GET presign）两步。

---

## 租户授权模型与跨租户隔离

本节介绍该解决方案的租户授权模型，包括授权字段、grant/revoke 操作、hub 侧授权决策、控制面 RBAC 与 owner 检查，以及跨租户隔离机制。
...
本节介绍该解决方案在接入时需要诚实标注的能力边界。下面这些能力要么是历史残留、要么是示例脚手架、要么是默认关闭的可选开关，接入方不应按生产级直接依赖。
...
> **Important**
>
> 接入方如需对外聊天端点，正确做法是按需做成 per-tenant 开关（只给确实需要的租户在重建时注入 `enabled:true`），不要全局打开。该端点是直通 LLM、绕过 control UI 设备认证的旁路，全局开等于给每个租户多一个对外攻击面。该 per-tenant 开关已实现：注册时传 `chat_endpoint_enabled:true`（默认 false，`deploy/lambda/api/handler.py:1309`）的租户，其 DynamoDB 记录置位（`handler.py:1509/1640`）并作为参数传给启动脚本（`handler.py:1691`、注入判定 `handler.py:3431`），启动时才注入 `chatCompletions.enabled:true`；未置位的租户仍是显式删除该端点，默认安全。
...
| `chatCompletions` 端点                                                                                                | 想要对外聊天 HTTP 端点的接入方                                                                                                                               | 默认显式删除（仅注册时传 `chat_endpoint_enabled:true` 的租户注入） | per-tenant 开关已实现（`handler.py:1309/1509/1691/3431`），按租户开，不要全局开 |
