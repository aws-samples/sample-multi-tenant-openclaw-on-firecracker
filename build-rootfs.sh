...
# The reference production sandbox runs Wazuh syscheck (FIM) + logcollector +
...
# 不带 hooks 键 ② templates/openclaw.json.example 不含 heartbeat.isolatedSession
# /lightContext、compaction.midTurnPrecheck/maxActiveTranscriptBytes。
# 升级 OpenClaw = 改这里版本号 + 重跑 2.26/目标版 schema 校验 + 验 launch-vm jq → 重烤。
...
# ── HARDENING (privilege drop) — our adaptation of the reference CapBnd=0 ──
# The reference production sandbox runs the agent with CapBnd=0000…0000
...
# The reference sandbox pins each instance to memory.max=4GB + cpu.max=2.5cores at the cgroup
...
# the reference cgroup sub-limit from decoration into enforcement.
...
# (tenant-scoped read), summarize, and market-data (read-only quotes). An attacker
# editing an analysis/ranking skill to fabricate a "safe" verdict or a fake board
# is a risk, so they are baked read-only + FIM-monitored like the other
# safety-relevant skills. The cp loop below (`[ -d ]` guard) tolerates a missing
# directory by skipping it with a warning rather than crashing.
