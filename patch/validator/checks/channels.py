...
def check_f2(ctx):
    ...
    if key:
        try:
            ctx.aws.call("s3", "head_object", Bucket=bucket, Key=key)
            object_present = True
    ...
    return finding(
        ...
        {"inspected": 1, "version": bootstrap["version"],
         ...
         "object_present": object_present, "active_token_lines": tokens,
...
def check_f3(ctx):
    ...
    for relative in required:
        ...
        for host in hosts:
            observed = (
                (host_hashes.get(host["instance_id"]) or {}).get(
                    "hashes", {}).get(destination)
