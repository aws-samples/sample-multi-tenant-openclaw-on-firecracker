...
```
ClawPool (this repo)
├── the pool          ← control plane, scheduler, hub, HA, observability   (your platform)
...
### Data plane: outbound channel, not inbound port-forward

The single most important design choice in the pool is **how the browser reaches a tenant's agent**. ClawPool does **not** open an inbound port on each microVM and reverse-proxy to it. Instead, each agent **dials out** and subscribes to the WebSocket Hub (a signed, Cognito-sub-bound channel); the Hub pairs the verified browser session with that outbound channel. The VM exposes **no inbound listener at all**.

This one decision pays off on four fronts:

- **🔒 Security** — The old model required opening an inbound port per tenant VM and reverse-proxying to it — one inbound attack surface per tenant, multiplied by thousands. The channel model means **the VM opens no inbound port**: there is nothing to scan, nothing to reach, nothing to forward to. Combined with cross-tenant `FORWARD DROP` (100% packet loss, measured), a tenant is unreachable from anywhere except the LLM path it dials out to itself.
- **⚡ Performance** — The Hub multiplexes one WebSocket per host and fans out to tenant channels, instead of maintaining N reverse-proxy upstreams with per-tenant nginx rules and health checks. Routing is a hash lookup on a live subscription, not a config rewrite. wss is a persistent connection — no per-message TLS/HTTP handshake on the hot path (streaming replies, market pushes).
- **🛠 Operations** — Because the agent re-subscribes on its own, **OpenClaw becomes effectively stateless**. When a VM is rescheduled to another host, restarted, or cold-started, the control plane does **not** have to discover its new IP/port and rewrite forwarding rules — the agent simply dials back out to the Hub and re-registers. Boot, drift, migrate, restart all reduce to "the agent reconnects." There is no inbound routing table to keep in sync with where VMs actually live.
- **🧰 Maintainability** — Auth is unchanged and pluggable, and **AWS Cognito is the single trust root**: identity is the **server-verified Cognito sub** (the Hub validates the Cognito-signed `id_token` before pairing a browser session), the channel signs with a per-tenant HMAC (`appId:appSecret`), and the secret never reaches the browser. An external OIDC IdP federates **into** the same Cognito User Pool rather than replacing it. Fewer moving parts (no per-tenant proxy config) means fewer things to break and a smaller blast radius when something does.

> In short: inbound port-forwarding couples the control plane to _where each VM is_. The outbound channel decouples them — the pool scales and self-heals because agents find the Hub, not the other way around.
...
DATA PLANE — consumer chat traffic (one independent-kernel microVM per tenant)
  End user (in your app / browser)
     │  wss long-lived (Cognito token in connect; not HTTP req/resp)
     ▼
  CloudFront (sole public ingress · WAF · security headers)
     ▼
  ALB (private to corp · IP allowlist · WS upgrade passthrough)
     ▼
  Amazon EKS · WebSocket Hub  ── per-tenant routing + fan-out, verified-sub cross-tenant isolation
     ▲  (each host: one WS multiplexed; tenant channels subscribe upstream VM→hub)
     │
  EC2 Auto Scaling Group · openclaw-hosts-asg
     host = r8g.metal-24xl (96 vCPU / 768 GB · native KVM) · 380 microVMs/host
...
- **Inbound**: Browser → CloudFront → ALB → WebSocket Hub (wss). The Hub pairs the verified-Cognito-sub frontend with the tenant's **outbound** channel (the VM dials out to register) — no inbound reverse-proxy to any VM port. Admin REST goes via API Gateway.
...
│   ├── userdata/
│   │   ├── init-host.sh       # Host init (pulls + starts the local hub in metal mode)
...
│   │   ├── install-hub.sh     # Install + start the WebSocket Hub on a host
│   │   └── stop-vm.sh         # microVM stop
│   ├── hub/                   # WebSocket Hub source (outbound-channel pairing; one mux WS per host)
│   └── eks/                   # Hub on EKS (cluster mode — per-tenant channel routing + fan-out)
...
│       └── security/   #   security layers baked in (claw-channel + sentinel/acl-guard)
│
│  ── SHARED ──
...
| **Encryption in transit**  | Browser → CloudFront → ALB → WebSocket Hub (TLS). The Hub then relays over the VM's own **outbound** channel — the VM dials out and registers (HMAC `appId:appSecret`); the ALB never reverse-proxies inbound to a VM port. Browser identity = server-verified Cognito sub; the browser never sees any channel/gateway secret.                                                                                                                     |
