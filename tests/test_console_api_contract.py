...
# The admin console was refactored from a single monolithic index.html into
# index.html + per-domain Alpine modules under console/js/app.*.js (tenant
# create/validation, role ranking, monitoring/system-info, migrate flow, the
# refresh() loop, etc.). The console–API *contract* is a property of the console
# as a whole, so the cross-file constants (name regex, role table, endpoint
# calls, refresh body) now live in those modules, not in index.html. Grep the
# concatenated bundle for those, and keep the pure-HTML structural checks (nav
# tabs, x-show blocks, the maxlength attribute) against index.html itself.
_JS_DIR = ROOT / "console" / "js"
_JS_SRC = (
    "\n".join(p.read_text() for p in sorted(_JS_DIR.glob("app*.js")))
    if _JS_DIR.is_dir()
    else ""
)
CONSOLE_SRC = INDEX_HTML + "\n" + _JS_SRC
...
class TestNameRegexParity:
    ...
    def test_console_has_a_name_regex(self):
        ...
        assert m, (
            "console missing a /^[a-z0-9](...)?$/ regex literal (index.html or js/app.*.js)"
        )
    ...
    def test_console_regex_matches_api_regex(self, handler):
        ...
        assert re.search(re.escape(api_pattern), CONSOLE_SRC), (
    ...
    def test_max_length_in_sync(self, handler):
        ...
        assert 'maxlength="32"' in INDEX_HTML, (
            "create modal name input missing maxlength=32"
        )
        assert "length > 32" in CONSOLE_SRC, (
            "console missing the > 32 character length guard"
        )
...
class TestRoleHierarchyParity:
    ...
    def test_console_has_same_role_keys(self):
        ...
        for role in ("viewer", "operator", "admin"):
            assert re.search(rf"{role}\s*:\s*[0-2]", CONSOLE_SRC), (
...
class TestNewEndpointsWired:
    ...
    def test_console_calls_system_info(self):
        assert re.search(
            r"this\.api\(['\"]GET['\"],\s*['\"]system/info['\"]", CONSOLE_SRC
        )

    def test_console_calls_agentcore_tools(self):
        assert re.search(
            r"this\.api\(['\"]GET['\"],\s*['\"]agentcore/tools['\"]", CONSOLE_SRC
        )

    def test_migrate_endpoint_used_by_console(self):
        """Console's migrate modal posts to /tenants/{id}/migrate."""
        assert (
            "tenants/' + id + '/migrate'" in CONSOLE_SRC
            or "tenants/' + id + '/migrate" in CONSOLE_SRC
        )
    ...
    def test_refresh_loads_system_info(self):
        ...
        m = re.search(r"async\s+refresh\s*\(\s*\)\s*\{([^}]*)\}", CONSOLE_SRC)
        assert m, "could not locate refresh() in console (index.html or js/app.*.js)"
        body = m.group(1)
