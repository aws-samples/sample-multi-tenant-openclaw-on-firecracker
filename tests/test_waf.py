...
class TestWafCustomConfig:
    ...
    def test_no_managed_rules_when_empty_list(self, template):
        """Empty managed_rules → 仍强制两条 baseline（task #25 安全加固：SQLi +
        IpReputation 不可被 config 裁掉）+ 一条 rate-based = 3 rules total。"""
        ...
        # 2 条不可裁的 managed baseline + 1 条 rate-based
        assert len(rules) == 3
        rules_str = str(rules)
        assert "AWSManagedRulesSQLiRuleSet" in rules_str
        assert "AWSManagedRulesAmazonIpReputationList" in rules_str
        assert any("RateBasedStatement" in str(r.get("Statement", {})) for r in rules)
