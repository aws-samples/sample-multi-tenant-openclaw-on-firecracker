...
Firecracker microVM 挂载四块盘，按 Firecracker PUT 顺序分配 `/dev/vd<N>`，root 固定为 `vda`，四块盘的文件系统均为 ext4：
...
本节介绍该解决方案的纵深防御安全模型。该模型由五层叠加构成，每一层都落到代码或 AWS 设备上，不依赖模型自律：L1 内容层（Amazon Bedrock Guardrails）、L2 工具执行层（sentinel-guard / acl-guard）、L3 身份层（Amazon Cognito + RBAC + owner 门控）、L4 网络层（iptables 三类 DROP + DNS Firewall）、L5 凭据·只读·监控层。物理与网络隔离管「租户之间」，工具执行层与内容层管「一个 agent 被诱导或被注入后能干什么」。
...
> **Note**
>
> 对「无显式 owner」节点（`owner_id` 为 `api-key` 或空）的共享放行受 `CLAW_HUB_SHARED_TENANT_ACCESS` 门控、默认关，关时这类节点对前端用户一律拒（fail-closed）。RBAC 默认开启是 config-gated 能力中的例外。

### L4 网络层（iptables 三类 DROP + DNS Firewall）

本层在 host 上以 iptables 强制跨租户与跨边界的网络隔离（Firecracker 自身不过滤流量）。所有安全 DROP 都插到链顶，确保排在追加的 ACCEPT/MASQUERADE 之前。三类 DROP 为：① IMDS 隔离，在 FORWARD 链 DROP guest 到 `169.254.169.254` 与 `169.254.169.253` 的流量，防租户经 MASQUERADE 到达 host 的实例元数据服务盗取 host EC2 instance-profile 凭据；② 东西向 L2 隔离，FORWARD 链 DROP guest 到整个租户超网 `SUBNET_PREFIX/16` 的流量，防包被路由进另一个租户的 /30；③ 管理面隔离，INPUT 链 DROP guest 到 host 的 8899/9090/22 端口。出网 DNS 拦截由 Amazon Route 53 Resolver DNS Firewall 作为 L4 出网补充：在 DNS 解析层 BLOCK 已知 C2 与数据外泄域名，guest 解析这类域名直接拿到 NXDOMAIN。该层 config-gated（`security.dns_firewall_enabled`，默认 false），真实威胁情报域名清单为安全敏感数据、不入仓库，生产由运营从受控源另灌。
