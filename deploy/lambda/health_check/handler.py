...
def _reap_stuck_creating(now):
    ...
    for t in stuck:
        ...
        try:
            ...
            continue  # lost the race (now running/deleted) — skip
...
def _advance_migration(tenant, now):
    ...
    if phase == "restore":
        ...
        # #172 — 只减 SOURCE 计数;TARGET 的 slot(next_vm_num/used_*/vm_count)在
        # migrate 发起时已由 _reserve_migration_slot 的 CAS 原子占用,这里绝不能再递增
        # target 或绝对赋值 next_vm_num(defect B:重复计账 + `SET next_vm_num=target_vm_num+1`
        # 会覆盖迁移进行中(分钟级)并发 create 对 target 的 CAS 递增)。
        if source_host_id:
            try:
                hosts_table.update_item(
                    ...
                    # defect C — floor guard:只在够减时才减,防并发/重复 sweep 把账本扣成负。
                    ConditionExpression=(
                        "used_vcpu >= :v AND used_mem_mb >= :m AND vm_count >= :one"
                    ),
                    ExpressionAttributeValues={
                ...
                # CCF(已被减过/账本不足)或其它 → non-fatal,不重复扣、不崩 sweep。
