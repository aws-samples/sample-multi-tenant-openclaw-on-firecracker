...
# 入口态是操作受理后、消息尚在队列等执行槽时行上保留的健康值,因此排队到死线也不该
# 降级它。restore 含 failed 是因为 #659 把 failed+suspended_at 定为受控重入口,一次重入
# 排队死不能把该能力降级。create 不在这里:它入队前已写 creating,占位行没有健康值可保。
_ENTRY_STATUSES: Dict[str, Tuple[str, ...]] = {
    create_deadline.ACTION_SUSPEND: ("running", "stopped"),
    create_deadline.ACTION_RESTORE: ("suspended", "failed"),
}
...
def _reason_for(item: Dict[str, Any]) -> str:
    ...
    if action != create_deadline.ACTION_CREATE:
        # M7 已证明归因只能有一个判定点,所以排队/在飞必须在这里区分;写库形态再由
        # `_fence_failed` 按同一张 `_ENTRY_STATUSES` 表分支,不能各抄一份状态字面量。
        if item.get("status") in _ENTRY_STATUSES.get(action, ()):
            return create_deadline.REASON_DEADLINE_QUEUED
...
def _fence_failed(
    ...
    | 动作 | 除原因与时刻之外还写什么 | 为什么 |
    | --- | --- | --- |
    | create/suspend/restore(在飞) | `status=failed` | 中间态在 status 上,真实执行已发生 |
    | suspend/restore(排队) | **什么都不写** | 入口健康值必须保留,只记录本次尝试失败 |
    | suspend/restore(已流转) | **一个字不写** | 无世代锚时不能错杀另一轮操作的结果 |
        ...
        if status in _intermediates:
            if action in (
                ...
                update = (
                    f"SET {reason_attr} = :reason, "
                    f"{create_deadline.fail_at_attr(action)} = :iso, "
                    # `deadline_enforced_for` 是轮次锚(Codex 实现评审 #1):执行者的刹车
                    # 只认「for == 自己手里的 deadline」;执行者与围栏赛跑、执行者赢下收尾
                    # 时留下的残留记录(for 属旧轮)对新一轮自动无效,不需要受理路径去清。
            ...
            # 返回 raced,下一拍按在飞死处置,不能用排队结论覆盖已经开始的真实执行。
            update = (
                f"SET {reason_attr} = :reason, "
                f"{create_deadline.fail_at_attr(action)} = :iso, "
                "deadline_enforced_at = :now"
            ...
            # #663 —— finalize 不清 restore_deadline,常规档又没有 op_id 世代锚;正如
            # `_fence_row` 的 docstring 所证,同值死线分不开两次操作。行已流转时任何写入都
            # 可能把上一条成功收尾的结果错杀,所以宁可完全不写。
            print(
                f"[#663] fence-skip {tid}: action={action} 行上 status={status!r} "
                "既非中间态也非入口态 —— 这行已流转到别的形态(典型:同租户上一条"
                "消息已成功收尾而 finalize 不清死线字段),动它就是错杀。不写。"
            )
            return "raced", reason
