...
>
> 🔄 **2026-07-08 data-plane redesign**: the data plane was decentralized. The old hub-WS + `claw-channel` outbound dial + three-Cognito-identity model is superseded by a two-tier route (`CloudFront → ALB → OpenResty edge ASG → host DNAT → microVM gateway`) with a KMS-envelope-encrypted per-tenant token as the sole credential. See [`docs/aws-guide/13-data-plane-redesign.md`](docs/aws-guide/13-data-plane-redesign.md) (EN: [`docs/aws-guide-en/13-data-plane-redesign.md`](docs/aws-guide-en/13-data-plane-redesign.md)) for the full walkthrough and the panorama diagram. Sections below marked with 🔄 reflect the new model; the rest of this README still describes control-plane / L1–L5 / image / sample layers unchanged.
...
`./setup.sh <region> <profile>` brings up the full stack in 5 minutes — self-managed VPC (/20, 3 AZ, 3 NAT GW), host ASG, OpenResty EdgeASG, ALB (idle 3600 s), ElastiCache Multi-AZ Redis, API Lambda, DynamoDB (`tenants` + `openclaw-tenant-secrets` with 15-min TTL), KMS ClawPoolCMK, and AgentCore, all wired and ready. Cloud rootfs build means **no local Linux required** (works from macOS / Windows).
...
### External platform federation flow

External-platform users federate through their own IdP into a single Cognito User Pool; a Pre-Token-Generation Lambda injects `custom:tenant_user_id` / `custom:platform_id` before the `id_token` is issued. Authorization is always decided server-side at the hub (`owner_id` / `authorized_users` ledger), never trusting client self-report.

<p align="center">
  <img src="docs/arch-federation-en.png" alt="External SaaS platform federation — Cognito federation + Pre-Token-Gen claim injection, end-to-end sequence" width="96%"/>
</p>
...
### 🔄 Data plane: two-tier route with token-only auth (2026-07-08 redesign)

The single most important design choice in the pool is **how the browser reaches a tenant's agent**. After the 2026-07-08 redesign, ClawPool runs a **two-tier route** through an OpenResty edge fleet that looks up each tenant in Redis and forwards to the host's iptables DNAT into the microVM's gateway. There is one credential end-to-end: a per-tenant **KMS-envelope-encrypted `gateway_token`**.

```
Browser  ─ wss ─▶  Platform BE  ─ HTTP+SSE, Bearer gateway_token ─▶
   CloudFront (origin readTimeout 180s, hard cap)
     ─▶ ALB (idle_timeout 3600s, SG allowlists CloudFront prefix list only)
       ─▶ OpenResty EdgeASG (3 AZ, min=3, ELB health, warmup gate)
         ─ Redis GET route:{tenant_id} (L1 lrucache 5s / L2 shared_dict 60s / L3 ElastiCache Multi-AZ)
           ─▶ Host iptables PREROUTING DNAT (ports 10000–10400, one per microVM)
             ─▶ microVM :18789  OpenClaw gateway, token-only, controlUi disabled
```

Design intent, in one line each:

- **🔒 Security** — one credential, one auth mode. The gateway trusts `Authorization: Bearer <gateway_token>` and nothing else (`controlUi.enabled=false`). The token is minted with KMS `GenerateRandom` (32 B), envelope-encrypted with `EncryptionContext={tenant_id}`, stored ciphertext-only in a DDB table with a 15-minute TTL, and cold-injected as an argv position into `launch-vm.sh` so plaintext never touches SSM, CloudTrail, or Lambda logs. The API Lambda **has no `kms:Decrypt` permission**; only the caller (the platform back end) decrypts, in its own process, with the same encryption context.
- **⚡ Performance** — routing is an in-memory hash on the edge. L1 (per-worker lrucache, 5 s) covers steady state; L2 (`ngx.shared.DICT`, 60 s) survives a Redis brownout via fail-static; L3 (ElastiCache Multi-AZ, `automatic_failover_enabled`) is the source of truth. Streaming replies flow as SSE with `proxy_buffering off`, unchanged end-to-end from the microVM. CloudFront's `readTimeout=180s` is a hard AWS cap, so **clients must ping every ~30 s** to keep an idle WS alive.
- **🛠 Operations** — no more hub, no more `claw-channel`, no more per-tenant Cognito app-client. Failover reduces to primary-endpoint DNS re-resolution (30 s TTL) plus L2 fail-static. Each host advertises its own `host_private_ip:host_port` into Redis via a strict port bitmap (10 000–10 400) so the edge routes to the right box even when a tenant moves.
- **🧰 Maintainability** — customer OIDC that cannot do headless login is no longer in the path. The identity chain is: user → platform JWT → `revealToken` returns KMS ciphertext → platform BE decrypts → HTTP `POST /ws/{tenant_id}/v1/chat/completions` upstream. External SSO federates into the platform's own OIDC provider, transparent to ClawPool.

> **Full walkthrough** — see [`docs/aws-guide/13-data-plane-redesign.md`](docs/aws-guide/13-data-plane-redesign.md) (EN: [`docs/aws-guide-en/13-data-plane-redesign.md`](docs/aws-guide-en/13-data-plane-redesign.md)), which includes the panorama topology, the end-to-end sequence, and the HA audit.
...
```
DATA PLANE — consumer chat traffic (one independent-kernel microVM per tenant)  🔄 2026-07-08
  End user (in your app / browser)
     │  wss long-lived to the platform back end (platform-signed JWT in ?token=)
     ▼
  Platform BE  ── revealToken → kms:Decrypt(EC={tenant_id}) → HTTP+SSE relay upstream
     │  POST /ws/{tenant_id}/v1/chat/completions  ·  Authorization: Bearer <gateway_token>
     ▼
  CloudFront  (sole public ingress · origin readTimeout 180s hard cap · client heartbeat ≤30s)
     ▼
  ALB  (internet-facing · idle_timeout 3600s · SG allowlists CloudFront prefix list only)
     ▼
  OpenResty EdgeASG  ── L1 lrucache 5s / L2 shared_dict 60s / L3 Redis GET route:{tenant_id}
     │  balancer_by_lua picks host:host_port (cross-host) or guest_ip:18789 (local)
     ▼
  EC2 Auto Scaling Group · openclaw-hosts-asg  ── iptables PREROUTING DNAT (ports 10000–10400)
     host = r8g.metal-24xl (96 vCPU / 768 GB · native KVM) · 380 microVMs/host  · nf_conntrack_max=1M
        ├── microVM tenant-A  (read-only ext4 golden image + overlay · 2 GB · cap-drop · L1 Guardrail · L2 sentinel · Wazuh/auditd · zero AWS creds)
        │       OpenClaw gateway :18789  ── token-only, controlUi disabled
        ├── microVM tenant-B  ...
        └── × 380
              │  LLM inference (agent → model)
              ▼
        LiteLLM gateway (per-tenant vkey metering/quota) ──→ Amazon Bedrock (PrivateLink, no public egress)

ROUTE AUTHORITY — ElastiCache Redis (Multi-AZ, automatic_failover)
  route:{tenant_id} = {"host":<host_private_ip>, "port":<host_port>, "guest_ip":<172.16.x.y>}
  Writer: host-agent (double-writes DDB descriptor + Redis on VM promotion)
  Reader: OpenResty edge (three-tier cache with fail-static)

  EGRESS REVIEW (every outbound hop inspected; host originates, layered to VPC edge):
     host iptables (cross-tenant + IMDS DROP · MASQUERADE)
       → Route53 DNS Firewall (threat list → NXDOMAIN, config-gated)
       → [planned] AWS Network Firewall (TLS_SNI allowlist · drop_strict) — not yet deployed
       → NAT Gateway → internet (allowlist only, else REJECT)

CONTROL PLANE — fully-managed serverless (no ops servers)
  Admin → POST /tenants → API Gateway /v1 (x-api-key)
        → Lambda openclaw-api (Cognito JWT verify + RBAC)
        → DynamoDB ×4 (tenants/hosts/groups/audit · CAS atomic slot)
        → SSM SendCommand (sole command channel → host)
        → host launch-vm.sh (start/stop/rebuild microVM — host executes, never hot-patches a live VM)
  Side-cars (scheduling/support, off the main path):
     EventBridge (health/scaler/backup cron) · helper Lambda ×4 · S3 (console/image/backup) · IAM least-privilege
```

</details>

> **Deep dive:** for the full architecture and security design — trust model (Amazon Cognito as the single trust root), the L1–L5 defense-in-depth detail, the data-plane outbound-channel rationale, and the **known limitations** — see the AWS-style implementation guide under [`docs/aws-guide/`](docs/aws-guide/) (chapters 02–03; an English edition is under [`docs/aws-guide-en/`](docs/aws-guide-en/)).
...
- **Inbound**: Browser → platform back end → CloudFront → ALB (least-outstanding-requests) → OpenResty edge ASG (looks up the tenant's `host:port` in the Redis route table) → host iptables PREROUTING DNAT → microVM `:18789` OpenClaw-native gateway. The gateway trusts only a per-tenant `Authorization: Bearer <gateway_token>`; no inbound reverse-proxy reaches any other VM port. Admin REST goes via API Gateway.
...
│   ├── userdata/
│   │   ├── init-host.sh       # Host init (Firecracker + edge-routing prerequisites)
│   │   ├── host-agent.py      # VM health polling + DDB writes + balloon
│   │   ├── launch-vm.sh       # microVM launch (cold-inject persona/skills + gateway_token → read-only ext4 golden image + overlay)
│   │   ├── route_ops.py       # host-side routing: iptables DNAT (ports 10000–10400) + Redis route-table dual-write
│   │   └── stop-vm.sh         # microVM stop
│   └── edge/                  # OpenResty edge (route.lua queries the Redis route table: L1 lrucache 5s / L2 shared_dict 60s / L3 ElastiCache)
...
| **L3 · Identity**                            | Control-plane identity gate + east-west hard isolation                               | Cognito JWT RS256 verify → RBAC → `owner_id` gate (code `if`). Cross-tenant `iptables FORWARD DROP` — **100% packet loss, kernel-level hard cut**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Encryption in transit**  | Browser → platform back end → CloudFront → ALB → OpenResty edge ASG (TLS). The edge looks up the tenant's `host:port` in the Redis route table and forwards over the host iptables PREROUTING DNAT to the microVM `:18789` OpenClaw-native gateway; no inbound reverse-proxy reaches any other VM port. The gateway trusts only a per-tenant `Authorization: Bearer <gateway_token>` (KMS-envelope-encrypted, `EncryptionContext={tenant_id}`); the browser never sees the gateway token.                                    |
