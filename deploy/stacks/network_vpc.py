...
def build_network_vpc(self, ctx):
    ...
    # ========== VPC (P2b · #187 FR-10, the data-plane contract §6) ==========
    ...
    # ========== #212 R1:api.mode 三态解析(edge|private|both)==========
    ...
    _want_private_api = _api_mode in ("private", "both")
    _edge_deny_public = _api_mode == "private" or bool(
        _api_cfg.get("edge_resource_policy_deny_public", False) if _api_cfg else False
    )
    # 传给 lambdas.py 的 EDGE resource policy 挂载点(通过 ctx 回带)。
    ctx._api_mode = _api_mode
    ctx._edge_deny_public = _edge_deny_public
    ...
    # #212 R1:门控从 private_api_enabled 布尔位改成 _want_private_api(api.mode 派生)。
    if _want_private_api:
        ...
        cdk.CfnOutput(self, "ExecuteApiVpceId", value=_execute_api_vpce.vpc_endpoint_id)
