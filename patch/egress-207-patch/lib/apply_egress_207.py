...
# ============================================================== lambda-api-code
LAMBDA_REL = "lambda/api/services/egress_admin_service.py"
LAMBDA_SRC = "deploy/lambda/api/services/egress_admin_service.py"
LAMBDA_IN_PKG = "services/egress_admin_service.py"
...
def build_overlay(fn: str, dest: Path) -> tuple[str, str]:
    """Download the live package and replace ONLY this kit's file. Returns (live_codesha, new_codesha).
    ...
    """
    import zipfile
    src = shipped_file(LAMBDA_REL, LAMBDA_SRC)
    ...
    if LAMBDA_IN_PKG not in names:
        raise Fail(f"{LAMBDA_IN_PKG} is not in the live package (its root holds "
    ...
    with zipfile.ZipFile(dest, "w", zipfile.ZIP_DEFLATED) as zo:
        for info in zin.infolist():
            data = body if info.filename == LAMBDA_IN_PKG else zin.read(info)
...
def op_lambda(mode: str) -> None:
    ...
    if mode == "verify":
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if LAMBDA_IN_PKG not in z.namelist():
                raise Fail(f"{LAMBDA_IN_PKG} is absent from the deployed package")
            got = hashlib.sha256(z.read(LAMBDA_IN_PKG)).hexdigest()
        if got != want_sha:
            raise Fail(f"the deployed {LAMBDA_IN_PKG} hashes to {got}, this kit ships {want_sha}; "
                       "this operation is NOT applied")
        log(f"asserted the deployed {LAMBDA_IN_PKG} is byte-identical to this kit's copy")
        if saved:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if codesha_want == codesha_before:
            ...
            receipt("openclaw-api", f"{LAMBDA_IN_PKG} already deployed (CodeSha256 {codesha_before})")
            return
        ...
        receipt("openclaw-api", f"{LAMBDA_IN_PKG} replaced; CodeSha256 {codesha_before} -> "
                                f"{codesha_want}; version {published} published"
                                + (f"; alias {alias} -> {published}" if alias else ""))


# ================================================================== edge-bundle
EDGE_REL = "host-scripts/edge/test/integration/balancer_phase_integration.sh"
EDGE_SRC = "deploy/edge/test/integration/balancer_phase_integration.sh"
EDGE_IN_BUNDLE = "test/integration/balancer_phase_integration.sh"
...
def op_edge(mode: str) -> None:
    ...
    # apply
    src = shipped_file(EDGE_REL, EDGE_SRC)
