...
_launch_one() {
  ...
  local gw_token_ct="${6:-}" device_paired="${7:-}"
  ...
  # 13 positional args mirror the launch-vm.sh signature: chat_ep=$10, position
  # 11 (cognito, retired #187 P5) stays empty, gateway_token_ct=$12 (#187 P1),
  # device_paired_b64=$13 (#188 wss 免 approve). Empty strings for positions 5-9
  # (and empty 12/13) keep launch-vm's backward-compat defaults — launch-vm.sh
  # fail-opens on empty 12/13 (openssl gateway token / no paired.json write).
  # Secrets like vkey/channel_secret are still pulled from DDB by launch-vm.
  bash "${LAUNCH_SH}" "${tid}" "${vm_num}" "${vcpu}" "${mem_mb}" \
    "" "" "" "" "" "${chat_ep}" "" "${gw_token_ct}" "${device_paired}" \
    >/dev/null 2>&1
  return $?
}
...
_dispatch_line() {
  ...
  local tid vm_num vcpu mem_mb chat_ep gw_token_ct device_paired
