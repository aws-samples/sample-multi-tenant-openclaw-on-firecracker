...
> 本章描述 2026-07-08 数据面去中枢化改造后的实时聊天链路。**代替** [03-架构详情](03-architecture-details.md) 里"实时聊天中枢 claw-hub"一节的旧模型(hub-WS + claw-channel 出站拨号 + Cognito 三处身份 + HMAC channel_secret 等)。改造缘由与影响面见 `engineering/00-knowledge-base/decisions/DECISION-drop-oidc-cognito-use-openclaw-native-auth.md`;权威接口契约见 `engineering/00-knowledge-base/SPEC/11-ENGINE-TRANSFORM/INTERFACE-CONTRACT.md`。运维手册(监控、告警、故障排查)见 [第 11 章 · 组件运维手册](11-ops-maintenance.md),本章不重复。
...
平台后端 WebSocket 网关
  ...
  -> host:host_port (跨机 DNAT) 或 guest_ip:18789 (本机)
  -> microVM OpenClaw gateway :18789
     (Ed25519 device 握手 + gateway token)
```

代码落地:OpenResty 侧 `deploy/edge/nginx.conf`、`deploy/edge/route.lua` 与 `deploy/edge/lib/backend.lua`;host 侧 `deploy/userdata/host-agent.py` 与 `deploy/userdata/route_ops.py`;IaC 侧 `deploy/stacks/ha_edge.py` 构建 Edge ASG、ElastiCache 与 ALB。

## 13.2 身份认证(device + token)

OpenClaw gateway 同时验证 per-tenant gateway token 与 Ed25519 device 签名。两者都是第二跳必需条件,不是二选一。凭据生命周期:

1. **铸造(mint)**:控制面 `POST /tenants` 时,`deploy/lambda/api/services/tenant_service.py` 的 `mint_gateway_token` 走 KMS GenerateRandom 32B → base64url 编码 → `kms_envelope.encrypt_with_tenant(plaintext, tenant_id, ClawPoolCMK)` 信封加密,EncryptionContext={"tenant_id":<id>} → 存 `openclaw-tenant-secrets` DynamoDB 表。#353 起该密文无 TTL,随租户生命周期长存。
2. **注入 microVM**:密文以 launch-vm 位置 12 参数传入 `deploy/userdata/launch-vm.sh`,host 侧 KMS 解密后写入 microVM 的可写 `data.ext4` 中 `openclaw.json .gateway.auth.token`。该 ext4 文件实际由 host 存储和挂载,因此 host compromise 可读取明文;明文不进入 SSM 命令或 CloudTrail 请求参数。
...
4. **生命周期**:密文表显式禁用 DynamoDB TTL。这样 1–2 年后的 rebuild、recover 或 restore 仍能复用原 gateway token 与 device 身份,不会因过期回退到另一套 token。删除租户时的保留/清理策略按控制面删除契约执行。
...
**三级路由缓存**(`deploy/edge/lib/backend.lua`)。本章统一称“ElastiCache 路由存储”;`config.yml` 默认引擎是 Valkey 7.2,也兼容 Redis OSS 7.1,两者使用相同 Redis wire protocol:
...
| L3  | ElastiCache Valkey/Redis `GET route:{tenant_id}` | —       | 权威源(host-agent 双写)                                   |
...
- **端口段**:`[10000, dnat_port_high]`(默认上界 15000 = 5001 槽,> 单 host 内存维度理论上限,消除端口先于内存耗尽;上界从 config 读、SG 与位图同源;`INTERFACE-CONTRACT §3`)。
...
**关键**:长静默 WebSocket 不能只依赖 ALB/OpenResty 的 3600s 配置;平台网关必须按自身 heartbeat 配置持续发送 WebSocket ping/pong,并在断线后重新认证和重连。不要把 HTTP/SSE read timeout 当成 WebSocket 业务完成判据。

## 13.6 十万级规模化基线(设计目标)

10 万 microVM · 约 300 host(按 380/host 的 2 GB 目标档推算,非默认实测容量)· 30 万并发 WS · 3 台 edge 起步。核心约束:
...
- **NFR-2 首回复时延**:T0(VM 启动完)→T2(首个 WebSocket `reply_delta`)的目标值需在目标环境重新测量;路由缓存命中只是其中一段。
- **NFR-1 接管**:单 edge 挂,余 2 台 LOR 平滑接管;客户端重连 ≤ 15s。ASG `health_check_grace_period=300s` 给冷启 openresty apt 装 + Lua warmup 全过程用。
- **NFR-4 隔离**:跨租户 100% 丢包(SPEC + FACT-BASELINE 实测);edge 路径不引入新越权面,第二跳必须同时通过 gateway token 与 Ed25519 device 握手。
...
| --------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 前端 → microVM  | wss `/hub/ws` → hub → claw-channel 出站 → agent                                               | wss `/gw/ws`(平台后端) → WebSocket → CloudFront → ALB → OpenResty → microVM gateway                |
| microVM 到 host | claw-channel 主动出站(零入站)                                                                 | 接入 `iptables PREROUTING DNAT` 从 host_port 转发到 :18789(microVM 只对 host 内部 tap 暴露 18789) |
| 身份            | Cognito × 3(user pool + 入口/channel appclient + custom:tenant_user_id) + HMAC channel_secret | gateway token + Ed25519 device 签名(KMS 密文由平台后端解密)                                       |
| 中转层          | claw-hub(自建 WS 中枢,EKS 上 3 pod)                                                           | OpenResty edge ASG(3 台 EC2,独立 ASG)                                                             |
| 路由权威源      | hub 内存 + owner 校验                                                                         | ElastiCache Valkey/Redis `route:{tenant_id}` 三级缓存                                             |
