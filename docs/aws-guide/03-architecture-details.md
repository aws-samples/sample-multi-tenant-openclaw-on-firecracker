...
本节介绍该解决方案的纵深防御安全模型。该模型由五层叠加构成，每一层都落到代码或 AWS 设备上，不依赖模型自律：L1 内容层（Amazon Bedrock Guardrails）、L2 工具执行层（sentinel-guard / acl-guard）、L3 身份层（Amazon Cognito + RBAC + owner 门控）、L4 网络层（iptables 三类 DROP + 出网默认拒绝白名单）、L5 凭据·只读·监控层。物理与网络隔离管「租户之间」，工具执行层与内容层管「一个 agent 被诱导或被注入后能干什么」。
