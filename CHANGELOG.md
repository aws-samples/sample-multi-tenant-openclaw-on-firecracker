...
Closes the remaining open issues: scheduler spread + concurrency-safe provisioning (#77), a clear balloon-migration failure path (#72), an operations audit trail with a Console Logs tab (#71), and a hermetic test suite (#80). Deployed to the live ap-northeast-1 stack and verified end-to-end (see "Live verification" below).
...
### Live verification (ap-northeast-1, 2-host fleet, RBAC_ENABLED=true)
...
- **#72 balloon guard** *(this release; superseded by real migration in 1.5.9)*: live `POST /tenants/{id}/migrate` with `BALLOON_ENABLED=true` → immediate `409 {"reason":"balloon_enabled"}`, no SSM fired.
...
- **Migration/failover flip could rewind a host's `next_vm_num` (#77 follow-up).** The sweep and AZ failover wrote an absolute `next_vm_num` from a stale read; a concurrent tenant create that had already advanced the counter got it rewound, letting a later create hand out a duplicate `vm_num` (→ duplicate guest_ip/host_port). The counter is now raised forward-only via a ConditionExpression.
- **Terraform path scheduler-env parity (#77 follow-up).** `terraform/main.tf` set no overcommit env vars, so the handler defaults silently diverged from the CDK stack. Added `cpu_overcommit_ratio` / `mem_overcommit_ratio` / `max_vms_per_host` variables wired into the api Lambda.
