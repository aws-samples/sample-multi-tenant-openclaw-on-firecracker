...
window.ocMonitoring = {
  ...
  queues: null, // R10.2 — SQS/DLQ depth (populated on Monitoring tab)
  ...
  // R10.2 — SQS/DLQ depth panel. Read-only counts (no message bodies).
  // dlq_alarm=true → operator should inspect stuck create/lifecycle actions.
  async loadQueues() {
    if (!this.apiUrl || !this.apiKey) return;
    try {
      this.queues = await this.api("GET", "system/queues");
    } catch {
      this.queues = null;
    }
  },
  ...
  skillS3Url(skill) {
    ...
    const bucket =
      this.systemInfo?.assets_bucket || window.OC_ASSETS_BUCKET || "";
    if (bucket && skill?.id) {
