...
def _write_action_deadline(tenant_id, action, deadline_epoch):
    ...
    **`attribute_exists(id)`**:同款理由 —— DDB 的 `update_item` 是 upsert,租户行真的会被
    ...
    try:
        clients.tenants_table.update_item(
            Key={"id": tenant_id},
            # #679 —— 受理新一轮操作时一并 REMOVE `deadline_enforced_at`(上一轮判死的
            # 记录)。它是刹车(`_deadline_brake_engaged`)与 reaper 补偿矩阵的触发标记,
            # 归位动作都会清它;这里再清一次是防极端残留的双保险 —— 一条陈旧的 enforced
            # 记录若滞留到新一轮,会让新一轮的刹车在 stop-vm 前误判"本轮已判死"而白白
            # 中止(deadline 值恰好重合时,常规档无世代锚分不开)。REMOVE 不存在的属性
            # 是 no-op,对没被判死过的行零影响。
            UpdateExpression=f"SET {attr} = :d REMOVE deadline_enforced_at",
            ConditionExpression="attribute_exists(id)",
...
def _deadline_brake_engaged(tenant_id, action, observed_deadline):
    ...
    **隐式刹车**(后续锚 `status=suspending` 的条件写因 failed 而 CCF)不复存在 ——
    ...
    真值表(强一致读,~毫秒级;enforced = 行上 `deadline_enforced_at`,
    match = 行上 `<action>_deadline` 与执行者手里 `observed_deadline` 相等):

      | enforced | match | 决策 | 含义 |
      | 在       | *     | 停   | 围栏记过账 —— 判的是本轮就该停;不匹配说明行已是新一轮,我是陈旧执行者,更该停 |
      | 不在     | 否    | 停   | 没判死但行已属新一轮(同租户新操作已受理)—— 我是陈旧执行者 |
      | 不在     | 是    | 放行 | 唯一的正常态 |
    ...
    if not row:
        ...
        return True
    if row.get("deadline_enforced_at") is not None:
        return True
