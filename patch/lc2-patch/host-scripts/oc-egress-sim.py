...
# 放行洞的绝对最小前缀长度。分两档,因为两种洞的责任方不同:
#   · LiteLLM CIDR(_ABSOLUTE_MIN_PREFIX = 24)是【平台配置】,烤进整批 host,
#     与 deploy/stacks/ha_edge.py::_EGRESS_ALLOW_MIN_PREFIX 同值同源(#575)。
#   · 运维经 API 加的 EGRESS_EXTRA_ALLOW(_ABSOLUTE_MIN_PREFIX_EXTRA_ALLOW = 16)
#     由调用方自己承担风险(#668),与控制面 admission
#     egress_admin_service._ABSOLUTE_MIN_PREFIX 同值同源。
# 两档都由 tests/test_594_... 的同源断言绑住。
#
...
# #668 只降运维洞这一档,平台配置的 LiteLLM CIDR 仍守 /24。
_DEFAULT_MIN_PREFIX = 24
_ABSOLUTE_MIN_PREFIX = 24
_ABSOLUTE_MIN_PREFIX_EXTRA_ALLOW = 16
...
def _min_allow_prefix(
    environment: Mapping[str, str],
    absolute_min: int = _ABSOLUTE_MIN_PREFIX,
) -> int:
    """放行洞的最小前缀长度:env 只能在 [absolute_min, 32] 里取值。"""
    raw = (environment.get(_MIN_PREFIX_ENV) or "").strip()
    prefix = _DEFAULT_MIN_PREFIX
    if raw:
        try:
            prefix = int(raw)
        except ValueError as error:
    ...
    return max(prefix, absolute_min)
...
def _parse_extra_allow(
    value: str,
    min_prefix: int = _ABSOLUTE_MIN_PREFIX_EXTRA_ALLOW,
) -> tuple:
...
def load_config(environment: Mapping[str, str]) -> EgressConfig:
    ...
    min_prefix_extra_allow = _min_allow_prefix(
        environment, _ABSOLUTE_MIN_PREFIX_EXTRA_ALLOW
    )
    min_prefix_litellm = _min_allow_prefix(environment)
    return EgressConfig(
        ...
        extra_allow=_parse_extra_allow(
            environment.get("EGRESS_EXTRA_ALLOW", ""), min_prefix_extra_allow
        ),
        litellm_cidrs=_parse_litellm_cidrs(
            environment.get("LITELLM_CIDR", ""),
            tenant_supernet,
            min_prefix_litellm,
        ),
...
def _selftest_cases() -> list[tuple[str, Packet, str]]:
    return [
        ...
        *[
        ...
        # 共享链跳转之前;#39 层开启时又把所有 :53 透明 DNAT 到 host dnsmasq
