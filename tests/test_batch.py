...
class TestBatchValidation:
    ...
    def test_both_ids_and_filter_rejected(self):
        ...
        assert resp["statusCode"] == 400
        assert (
            "exactly one" in json.loads(resp["body"])["error"].lower()
            or "both" in json.loads(resp["body"])["error"].lower()
        )

    @pytest.mark.unit
    def test_over_100_ids_goes_async_503_without_jobs_table(self):
        # >100 ids switches to the async-job path; with no BATCH_JOBS_TABLE
        # configured the request is rejected with 503 (not 400). See handler
        # batch_tenants: <=100 sync, >100 async (needs table), >100000 hard 400.
        ids = [f"t{i}" for i in range(101)]
        resp = _invoke_batch({"action": "stop", "ids": ids})
        assert resp["statusCode"] == 503

    @pytest.mark.unit
    def test_over_100000_ids_hard_rejected_400(self):
        ids = [f"t{i}" for i in range(100_001)]
        resp = _invoke_batch({"action": "stop", "ids": ids})
        assert resp["statusCode"] == 400
...
class TestBatchActionDispatch:
    ...
    def test_stop_dispatches_to_each_tenant(self):
        ...
        with patch.object(
            api,
            "tenant_action",
            return_value=api._resp(200, {"id": "x", "status": "stopped"}),
        ) as m:
    ...
    def test_start_dispatched(self):
        ...
        with patch.object(
            api,
            "tenant_action",
            return_value=api._resp(200, {"id": "t1", "status": "running"}),
        ):
    ...
    def test_backup_dispatched(self):
        ...
        with patch.object(
            api,
            "tenant_action",
            return_value=api._resp(202, {"id": "t1", "status": "started"}),
        ):
    ...
    def test_delete_uses_delete_tenant(self):
        ...
        with patch.object(
            api,
            "delete_tenant",
            return_value=api._resp(200, {"id": "t1", "status": "deleted"}),
        ) as m:
...
class TestBatchFailureIsolation:
    ...
    def test_action_exception_caught_per_tenant(self):
        ...
        api.tenants_table.get_item.side_effect = lambda Key, **kwargs: {
            "Item": _running_tenant(Key["id"])
        }

        def fake_action(tid, action, body=None, event=None):
            if tid == "boom":
...
class TestBatchFilterByTag:
    ...
    def test_filter_excludes_deleted_tenants(self):
        ...
        with patch.object(
            api,
            "tenant_action",
            return_value=api._resp(200, {"id": "a", "status": "stopped"}),
        ) as m:
