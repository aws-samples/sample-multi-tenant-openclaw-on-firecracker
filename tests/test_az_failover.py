...
class TestGroupHostsByAZ:
    ...
    def test_skips_hosts_without_az(self, hc):
        ...
        assert "az-a" in out
        assert all(
            "instance_id" in h and h["instance_id"] != "i-2"
        ...
        hosts = [
            {
                "instance_id": "i-1",
                "az": "az-a",
                "last_health_check": _ago_iso(20 * 60),
            },
            {
                "instance_id": "i-2",
                "az": "az-a",
                "last_health_check": _ago_iso(15 * 60),
            },
...
class TestPickTargetHost:
    ...
    def test_picks_host_with_most_spare_vcpu(self, hc):
        ...
        target = hc.pick_target_host(
            hosts, now, threshold_minutes=10, exclude_azs={"az-a"}
        )
    ...
    def test_excludes_unhealthy_host(self, hc):
        ...
        assert (
            hc.pick_target_host(hosts, now, threshold_minutes=10, exclude_azs=set())
            is None
        )
    ...
    def test_required_vcpu_filter(self, hc):
        ...
        assert (
            hc.pick_target_host(
                hosts, now, threshold_minutes=10, exclude_azs=set(), required_vcpu=4
            )
            is None
        )
        assert (
            hc.pick_target_host(
                hosts, now, threshold_minutes=10, exclude_azs=set(), required_vcpu=2
            )
            is not None
        )
    ...
    def test_no_candidates_returns_none(self, hc):
        now = datetime.now(timezone.utc)
        assert (
            hc.pick_target_host([], now, threshold_minutes=10, exclude_azs=set())
            is None
        )
    ...
    def test_deterministic_tiebreak(self, hc):
        ...
        target = hc.pick_target_host(
            hosts, now, threshold_minutes=10, exclude_azs=set()
        )
...
class TestCooldown:
    ...
    def test_no_prior_failover_proceeds(self, hc):
        now = datetime.now(timezone.utc)
        assert (
            hc.should_skip_az_for_cooldown({}, "az-a", now, cooldown_minutes=30)
            is False
        )
    ...
    def test_recent_failover_within_cooldown_skips(self, hc):
        ...
        assert (
            hc.should_skip_az_for_cooldown(state, "az-a", now, cooldown_minutes=30)
            is True
        )
...
class TestAZFailoverOrchestration:
    ...
    def test_no_outage_returns_clean_summary(self):
        ...
        hc.hosts_table.scan.return_value = {
            "Items": [
                {
                    "instance_id": "i-1",
                    "az": "az-a",
...
class TestFailoverTenant:
    ...
    def test_ssm_command_fails_with_nonzero_status(self):
        ...
        # SSM restore reported Failed; the verify gate then confirms NOT running.
        # _verify_vm_actually_running polls to a 120s wall-clock deadline before
        # returning False, so stub it — this test asserts the failover_failed
        # branch, not verify poll timing.
        with patch.object(hc, "_verify_vm_actually_running", return_value=False):
            ok = hc._failover_tenant_to_host(tenant, target, source_az="az-a", now=now)
        assert ok is False
...
class TestFindLatestBackupKey:
    ...
    def test_single_backup_returned_as_key_only(self):
        ...
        assert not key.startswith("s3://"), "must return S3 key only, no s3:// prefix"
...
class TestVerifyVmActuallyRunning:
    ...
    def test_returns_true_when_probe_outputs_VERIFIED(self):
        ...
        ssm.get_command_invocation.side_effect = [
            ...
            {
                "Status": "Success",
                "StandardOutputContent": "VERIFIED\n",
            },  # fetch stdout
        ]
        assert (
            hc._verify_vm_actually_running("i-new", "t1", timeout_sec=10, poll_sec=0)
            is True
        )
    ...
    def test_returns_false_when_probe_outputs_NOT_RUNNING(self):
        ...
        assert (
            hc._verify_vm_actually_running("i-new", "t1", timeout_sec=2, poll_sec=0)
            is False
        )
...
class TestSsmFailButVerifySucceeds:
    ...
    def test_ssm_fail_and_verify_fail_marks_failover_failed(self):
        ...
        # verify gate confirms NOT running. _verify_vm_actually_running polls to a
        # 120s wall-clock deadline before returning False (it intentionally gives
        # the VM time to come up), so stub it directly — the test asserts the
        # failover_failed branch, not the verify poll timing.
        with patch.object(hc, "_verify_vm_actually_running", return_value=False):
            outcome = hc._failover_tenant_to_host(
                tenant, target, source_az="az-a", now=now
            )
            ...
            "Rules": [
                {
                    ...
                    "Priority": "10",
                    "Conditions": [
                        {"Field": "path-pattern", "Values": ["/vm/t1", "/vm/t1/*"]}
                    ],
                    "Actions": [
                        {"Type": "forward", "TargetGroupArn": "arn:tg:oldhost"}
                    ],
                }
            ],
        }
            ...
            "Rules": [
                {
                    ...
                    "Priority": "20",
                    "Conditions": [
                        {"Field": "path-pattern", "Values": ["/vm/some-other"]}
                    ],
...
class TestBlockedSummaryBucket:
    ...
    def test_no_backup_increments_blocked_not_failed(self):
        ...
        result = hc._check_and_handle_az_failover(
            now=datetime.now(timezone.utc), tenants=tenants
        )
...
class TestVmNumAllocationBatch:
    ...
    def test_vm_num_increments_between_tenants(self):
        ...
        ssm = hc._test_mocks["ssm"]
        cmds = [
            c.kwargs.get("Parameters", {}).get("commands", [""])[0]
...
class TestCooldownIdempotency:
    ...
    def test_cooldown_persisted_even_when_no_tenants_to_migrate(self):
        ...
        result = hc._check_and_handle_az_failover(
            now=datetime.now(timezone.utc), tenants=[]
        )
        assert result["az_outages_detected"] == 1
        ...
        state_put = next(
            (
                c
                for c in put_calls
                if c.kwargs.get("Item", {}).get("instance_id")
                == "__az_failover_state__"
            ),
            None,
        )
