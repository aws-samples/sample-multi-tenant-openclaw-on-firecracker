...
| `rebuild`                       | rebuild with `image_channel`; defaults to `live`; the worker requires a successful backup before rebuilding                                                           | 202 async; poll `GET /tenants/{id}`                                                                     |
...
**`POST {BASE}/hosts/{instance_id}/taint`** — cordon a host (**api-key or RBAC operator+**; operations scripts holding an API key are the primary caller, so this route skips the pre-route role gate and is gated by an identity check inside the handler — viewer and unverified tokens always get 403, and a **platform-scoped API key also always gets 403**, since per #108 it may only touch tenants in its own platform while a taint acts on a fleet-wide host). Since #539, `is_tainted` is **orthogonal** to the lifecycle `status` and only expresses a NoSchedule operations intent: it does not touch the running instance or its tenants, it only marks the host so the scheduler avoids it during placement (exclusion logic is the read side, #540). body `{reason:"<non-empty reason>"}`. Idempotent: an already-tainted host returns `200 {..., already_tainted:true}` without overwriting the original `tainted_at/reason`. Returns `200 {instance_id,is_tainted:true,tainted_at,tainted_reason}`; a missing/empty `reason` returns `400 VALIDATION`, an unknown host returns `404` (and never creates a ghost row). The `tainted_by` audit field records the caller identity but is not surfaced on `GET /hosts`. `tainted_at` is an epoch-second **JSON number**, with the same type on both the first and the idempotent path (#546 fixed the drift where the idempotent path serialized DDB's `Decimal` to a string via `default=str`; strongly-typed clients can safely parse it as a number).
**`DELETE {BASE}/hosts/{instance_id}/taint`** — clear the taint (**api-key or RBAC operator+**, same as above). Only REMOVEs the taint attributes; it **never** persists `is_tainted:false` — the `GET /hosts` `is_tainted` is filled in as `false` by the response layer. Idempotent: an untainted host returns `200 {..., already_untainted:true}`. Returns `200 {instance_id,is_tainted:false}`.

> **Effect (#540 read side is merged into bb; the taint now affects placement).** Once marked, the scheduler excludes the host from **every** placement path: synchronous create (201) / restore / `process_pending`, queued create (202, **the production default path**), pinned create (`preferred_host_id`) → `409`, and migrate with an explicit `target_host_id` → `409` (always rejected — there is no force/allow_tainted escape hatch). The select-then-claim TOCTOU window is closed by a DynamoDB conditional write (`attribute_not_exists(is_tainted) OR is_tainted <> true`, dirty values fail open, same semantics as the read side); the ASG in-flight-capacity count also excludes tainted hosts (so cordoning a batch does not starve pending tenants by making scale-out think capacity is already on the way). It does **not** evict, stop, or migrate existing tenants on the host; operations broadcasts (`refresh-rootfs` / fleet action) and image operations (`pull-image` / `promote-canary`) still cover tainted hosts as usual.
>
> **How to use it.** `POST .../taint` to mark (with a `reason`) → `GET /hosts` and check `is_tainted:true` → new create/restore now automatically avoids the host → `DELETE .../taint` to clear once the operation is done.
>
> **Deployment prerequisite (important).** The placement exclusion is the read side (#540). **This behavior is only in effect where the #540 read-side code is deployed**: in an environment carrying only the write side (#539), the taint API still returns `200` with `is_tainted=true`, but the scheduler does **not** exclude — new tenants still land on the tainted host. Before relying on cordon, confirm the Lambda that performs placement (the create path: `openclaw-api` and the queue consumer) ships a deployment package whose `core/scheduling.py` has the `host_taint.is_tainted` exclusion and whose `core/dispatch/binpack.py` has the `taint_ok` gate; a 200 from the mark API alone is not enough to tell operators or customers that "cordon is in effect".
**`POST {BASE}/hosts/refresh-rootfs`** — legacy flat-layout hosts only: push the latest rootfs + data template + read-only disk from `manifest.json` through SSM to all active/idle hosts (RBAC operator+). Each host serializes under `pull.lock`, stages through `.tmp→mv`, and only then commits `rootfs_version/immutable_version`. No body. No hosts returns `200 {message:"no active hosts",updated:0}`. An SSM command id returns `202 {message:"refresh accepted",status:"accepted",command_id,version,immutable_version,hosts:[...]}`; this proves acceptance only, so poll SSM by `command_id` for terminal results. If any host has `image_slots`, the whole request fails closed with `409 INCOMPATIBLE_HOST_LAYOUT`; use `pull-image` + `promote-canary` instead. A response without an SSM command id returns `502 DISPATCH_UNCONFIRMED`.
...
**`PUT {BASE}/skills/{name}`** — write/create a shared skill (operator+): body `{content}`, must be UTF-8, ≤256KB, and contain at least one top-level `# heading`; returns `200` (already existed) or `201` (newly created) with `{name,size,created,version_id,etag,sha256,sync}`. After storing SHA-256 metadata in S3, the API immediately dispatches the version-aware syncer through SSM in batches of at most 50 hosts. `sync.status:"accepted"` means SSM returned command ids, not that every host has completed. Oversize or malformed content returns `400`.
**`DELETE {BASE}/skills/{name}`** — delete a shared skill (operator+): returns `{name,deleted:<number of files deleted>,sync}`; the syncer observes the latest S3 delete marker and removes the host copy.

> **Note** A skill name is limited to lowercase letters + digits + hyphens; an illegal name returns `400`. Host sync compares S3 `VersionId/ETag`, so same-size replacements are applied; bytes are verified before atomic replacement, and a failed download neither overwrites the old file nor advances state. PUT/DELETE use SSM for immediate propagation and the five-minute cron as reconciliation. A running microVM is not hot-patched; it adopts the new shared skill on its next restart/rebuild. Immutable skills, `AGENTS.md`, and persona still require rebuilding the immutable disk and following the image-upgrade path.
...
| `/hosts/{instance_id}/taint`                           | POST/DELETE    | api-key · operator         | set/clear cordon taint (NoSchedule, orthogonal to status; platform-scoped key 403)                |
