...
with patch("boto3.resource", return_value=_mock_ddb), patch("boto3.client") as _mc:
    ...
    spec = importlib.util.spec_from_file_location(
        "api_handler", "deploy/lambda/api/handler.py"
    )
...
def _host(
    total_vcpu=8, total_mem_mb=16384, used_vcpu=0, used_mem_mb=0, status="active"
):
    return {
        "instance_id": "i-test",
...
class TestFindHostMemOvercommit:
    @pytest.mark.unit
    @pytest.mark.skipif(
        not HAS_MEM_OVERCOMMIT, reason="mem_overcommit_ratio not implemented yet"
    )
...
class TestFindHostCombined:
    ...
    def test_both_must_fit(self):
        ...
        api.hosts_table.scan.return_value = {
            "Items": [_host(used_vcpu=4, used_mem_mb=24576)]
        }
    ...
    def test_no_overcommit_strict(self):
        ...
        try:
            ...
            api.hosts_table = make_ddb_table()
            api.hosts_table.scan.return_value = {
                "Items": [_host(used_vcpu=7, used_mem_mb=13000)]
            }
...
class TestFindHostSpreadAndMixedFleet:
    """Loop 2026-07-02 coverage gap: _find_host's load-spread ranking (pick the
    host with the most balanced headroom, not first-fit) and MIXED instance
    types (each host gated by its OWN total_vcpu/total_mem_mb). Existing tests
    only covered single-host overcommit fit/reject. Ratio pinned to 1.0 so the
    arithmetic is deterministic regardless of env."""

    def setup_method(self):
        self._ocpu = api.CPU_OVERCOMMIT_RATIO
        self._omem = getattr(api, "MEM_OVERCOMMIT_RATIO", 1.0)
        api.CPU_OVERCOMMIT_RATIO = 1.0
        if HAS_MEM_OVERCOMMIT:
            api.MEM_OVERCOMMIT_RATIO = 1.0

    def teardown_method(self):
        api.CPU_OVERCOMMIT_RATIO = self._ocpu
        if HAS_MEM_OVERCOMMIT:
            api.MEM_OVERCOMMIT_RATIO = self._omem

    @pytest.mark.unit
    def test_spreads_to_least_loaded_not_first(self):
        """Two identical hosts, first is half-full, second empty → must pick the
        EMPTY one (higher balanced-headroom score), NOT the first in scan order.
        Guards against the old 'return first fit' funnel-onto-one-host bug."""
        busy = _host(total_vcpu=8, total_mem_mb=16384, used_vcpu=4, used_mem_mb=8192)
        busy["instance_id"] = "i-busy"
        empty = _host(total_vcpu=8, total_mem_mb=16384)
        empty["instance_id"] = "i-empty"
        api.hosts_table = make_ddb_table()
        # scan returns busy FIRST — first-fit would wrongly pick it.
        api.hosts_table.scan.return_value = {"Items": [busy, empty]}
        result = api._find_host(2, 4096)
        assert result["instance_id"] == "i-empty"

    @pytest.mark.unit
    def test_mixed_fleet_each_gated_by_own_capacity(self):
        """Mixed instance types: a big host (96 vCPU/768GB) and a small host
        (8 vCPU/16GB). A request that only fits the big host must NOT be placed
        on the small one — each host's hard gate uses its OWN totals."""
        big = _host(total_vcpu=96, total_mem_mb=786432)
        big["instance_id"] = "i-metal"
        small = _host(total_vcpu=8, total_mem_mb=16384, used_vcpu=6)
        small["instance_id"] = "i-small"
        api.hosts_table = make_ddb_table()
        api.hosts_table.scan.return_value = {"Items": [small, big]}
        # needs 4 vCPU: small has only 8-6=2 free → must land on the metal host.
        result = api._find_host(4, 4096)
        assert result is not None
        assert result["instance_id"] == "i-metal"

    @pytest.mark.unit
    def test_mixed_fleet_small_host_still_usable_for_small_ask(self):
        """A small ask that fits the tighter (higher-ratio-free) small host is
        allowed there — mixed fleet doesn't force everything onto the big host."""
        big = _host(total_vcpu=96, total_mem_mb=786432, used_vcpu=90)  # nearly full
        big["instance_id"] = "i-metal"
        small = _host(total_vcpu=8, total_mem_mb=16384)  # empty → best ratio
        small["instance_id"] = "i-small"
        api.hosts_table = make_ddb_table()
        api.hosts_table.scan.return_value = {"Items": [big, small]}
        result = api._find_host(2, 4096)
        # empty small host has the most balanced headroom → chosen over near-full metal.
        assert result["instance_id"] == "i-small"
...
class TestListHosts:
    ...
    def test_includes_overcommit_ratios(self):
        api.hosts_table = make_ddb_table()
        api.hosts_table.scan.return_value = {
            "Items": [{"instance_id": "i-1", "status": "active", "vm_count": 0}]
        }
...
class TestCreateTenantRestore:
    ...
    def test_restore_latest_backup(self):
        ...
        _mock_s3.list_objects_v2.return_value = {
            "Contents": [
                {
                ...
                {
                    "Key": "backups/src/20260301-000000.gz",
                    "LastModified": datetime.datetime(2026, 3, 1),
                    "Size": 200,
                },
            ]
        }
        resp = api.create_tenant(
            json.dumps({"name": "restored", "restore_from": {"tenant_id": "src"}})
        )
        assert resp["statusCode"] == 201
    ...
    def test_restore_source_has_no_backups_returns_404(self):
        ...
        resp = api.create_tenant(
            json.dumps({"name": "restored", "restore_from": {"tenant_id": "src-gone"}})
        )
    ...
    def test_restore_when_source_tenant_deleted(self):
        ...
        _mock_s3.list_objects_v2.return_value = {
            "Contents": [
                {
                    "Key": "backups/src-deleted/20260101-000000.gz",
                    "LastModified": datetime.datetime(2026, 1, 1),
                    "Size": 100,
                },
            ]
...
class TestListAllBackups:
    ...
    def test_marks_orphan_vs_active(self):
        ...
        page = {
            "Contents": [
                ...
                },
                {
                    ...
                    "LastModified": datetime.datetime(2026, 2, 1),
                    "Size": 2000,
                },
                {
        ...
        assert [b["tenant_id"] for b in backups] == [
            "alive",
            "soft-deleted",
            "orphan-in-s3",
        ]
    ...
    def test_skips_non_gz_and_malformed_keys(self):
        ...
        page = {
            "Contents": [
                ...
                {
                    "Key": "backups/tenant/readme.txt",
                    "LastModified": datetime.datetime(2026, 1, 1),
                    "Size": 10,
                },  # not .gz
                {
                    "Key": "backups/tenant/20260101-000000.gz",
                    "LastModified": datetime.datetime(2026, 1, 1),
                    "Size": 100,
                },
            ]
...
class TestListTenants:
    ...
    def test_returns_200(self):
        api.tenants_table = make_ddb_table()
        # 真实租户必有 status + host_id；list_tenants 会丢弃缺 host_id 的半成品/幽灵行
        # （handler.py 的 "丢 ghost 行" 过滤），故 mock 数据补齐这两个字段。
        api.tenants_table.scan.return_value = {
            "Items": [{"id": "t1", "status": "running", "host_id": "i-abc"}]
        }
...
class TestRouting:
    ...
    def test_unknown_route_404(self):
        resp = api.lambda_handler(
            {"httpMethod": "GET", "resource": "/nope", "pathParameters": {}}, None
        )
        assert resp["statusCode"] == 404
    ...
    def test_cors_headers(self):
        ...
        api.tenants_table.scan.return_value = {"Items": []}
        resp = api.lambda_handler(
            {"httpMethod": "GET", "resource": "/tenants", "pathParameters": {}}, None
        )
