...
- **GET /audit-log** (RBAC viewer): lists audits (newest-first), with query parameters `limit` (default 50, max 500) and `since` (ISO-8601); returns 200 `[]` without an audit_table. Audit best-effort records all POST/PUT/DELETE; the Amazon DynamoDB table has a 90-day TTL after which entries are removed — this is a mutable audit trail, not immutable compliance evidence. A WORM-grade immutable audit trail is planned (not implemented, see issue #32).
