...
os.environ["NOTIFICATIONS_TOPIC_ARN"] = (
    "arn:aws:sns:us-east-1:123:openclaw-tenant-events"
)
...
class TestLifecyclePublish:
    ...
    def test_start_action_publishes(self):
        ...
        api.tenants_table.get_item.return_value = {"Item": _running_tenant("t1")}
        api.sns = MagicMock()
        # start runs synchronously via _ssm_run (SSM send + poll-to-deadline). The
        # mocked SSM never returns "Success", so the poll would spin to its timeout.
        # This test asserts the SNS lifecycle publish, not SSM polling — stub _ssm_run.
    ...
    def test_backup_action_publishes(self):
        ...
        with patch(
            ...
            side_effect=lambda svc, **kw: (
                fake_lambda if svc == "lambda" else MagicMock()
            ),
        ):
