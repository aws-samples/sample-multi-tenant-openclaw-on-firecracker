...
def _tenant(tid, tags=None, status="running"):
    item = {
        ...
        "name": tid,
        "status": status,
        # A real tenant always has a host assignment; list_tenants drops ghost
        # rows that lack host_id (handler.py ghost-row filter), so the fixture
        # must carry one or it gets filtered out before the tag-filter step.
        "host_id": "i-h1",
        "vcpu": 2,
        "mem_mb": 4096,
    }
