...
apt-get -o DPkg::Lock::Timeout=60 install -y -qq curl jq unzip pigz nginx python3-redis > /dev/null 2>&1
...
NGINX
# #187 转型:conf.d/tenants/*.conf include 保留(未来若两级路由 edge 需在 host
# nginx 层加辅助反代可就地扩,当前无内容,include 空目录无害)。claw-hub /hub/*
# 反代及 install-hub.sh 生成的 00-claw-hub.conf 已随 hub 下线归档。
