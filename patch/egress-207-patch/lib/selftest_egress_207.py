...
"""Self-test for apply_egress_207.py. Makes NO AWS calls: every call is served by a programmable
...
Usage: python3 lib/selftest_egress_207.py
"""
import base64
import gzip
import hashlib
...
def load_module():
    spec = importlib.util.spec_from_file_location("apply_egress_207", HERE / "apply_egress_207.py")
...
def test_kit_is_consistent_with_itself(m):
    ...
    for rel, src in ((m.LAMBDA_REL, m.LAMBDA_SRC), (m.EDGE_REL, m.EDGE_SRC)):
        p = KIT / rel
    ...
    bad["paths"][m.EDGE_SRC]["patch_sha256"] = "0" * 64
    tmp.write_text(json.dumps(bad))
    real = m.MANIFEST
    m.MANIFEST = tmp
    try:
        m.shipped_file(m.EDGE_REL, m.EDGE_SRC)
...
def test_overlay_replaces_exactly_one_entry(m):
    ...
    others = {"handler.py": b"# handler\n", "core/__init__.py": b"", "core/auth.py": b"# auth\n",
              ...
              m.LAMBDA_IN_PKG: b"# the OLD egress service\n"}
    ...
    try:
        ...
        check(differ == [m.LAMBDA_IN_PKG], f"[2] exactly the shipped file differs (got {differ})")
        check(hashlib.sha256(zc.read(m.LAMBDA_IN_PKG)).hexdigest()
...
def test_lambda_publishes_last_and_unwinds(m):
    ...
    with zipfile.ZipFile(live, "w") as z:
        ...
        z.writestr(m.LAMBDA_IN_PKG, b"# the OLD egress service\n")
    live_sha = m.zip_codesha(live)
    ...
    with zipfile.ZipFile(overlay_probe, "w", zipfile.ZIP_DEFLATED) as zo:
        for info in zin.infolist():
            data = src.read_bytes() if info.filename == m.LAMBDA_IN_PKG else zin.read(info)
...
def test_edge_apply_pins_and_unwinds(m):
    ...
    key = f"deployment/bootstrap/edge/{sha}/{m.EDGE_IN_BUNDLE}"
    world = {
        "autoscaling describe-auto-scaling-groups": {
            "LaunchTemplate": {"LaunchTemplateId": "lt-1", "Version": "3"}},
        "ec2 describe-launch-template-versions":
    ...
    shipped = (KIT / m.EDGE_REL).read_bytes()
...
def test_verify_reads_the_deployed_artifact(m):
    ...
    src = (KIT / m.LAMBDA_REL).read_bytes()
    td = Path(tempfile.mkdtemp())
    pkg = td / "deployed.zip"
    with zipfile.ZipFile(pkg, "w") as z:
        z.writestr("handler.py", b"# handler\n")
        z.writestr(m.LAMBDA_IN_PKG, src)
    world = {
...
def test_region_is_always_explicit(m):
    ...
    src = (HERE / "apply_egress_207.py").read_text()
...
def main() -> int:
    ...
    test_edge_prefix_is_discovered_not_assumed(m)
    test_edge_apply_pins_and_unwinds(m)
