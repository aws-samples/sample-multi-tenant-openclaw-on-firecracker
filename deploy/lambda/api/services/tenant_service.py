...
import core.legacy_alb as legacy_alb
...
def create_tenant(body=None, event=None):
    ...
    if clients.DISPATCH_QUEUE_URL and not (event or {}).get("_consumer_ident"):
        ...
        msg = {
            ...
            "params": {
                ...
                "mem_mb": int(mem_mb),
                "owner_id": owner_id,
                "chat_ep": bool(body.get("chat_ep", False)),
                "image": body.get("config_template") or "default",
            },
        }
        try:
    ...
    cognito_creds = auth._provision_channel_machine_user(tenant_id)
    ...
    tg_arn = legacy_alb._ensure_host_tg(host["instance_id"], host["private_ip"])
    legacy_alb._add_alb_rule(tenant_id, tg_arn)
...
def delete_tenant(tenant_id, query_params, event=None):
    ...
    # Remove ALB rule
    legacy_alb._remove_alb_rule(tenant_id)

    if host_id:
        # Remove DNAT rule (best effort)
        ssm_dispatch._ssm_run(
            host_id,
            f"sudo iptables -t nat -D PREROUTING -i $(ip route show default | awk '{{print $5}}' | head -1) -p tcp --dport {item.get('host_port', 0)} -j DNAT --to-destination {item.get('guest_ip', '')}:{clients.VM_PORT_BASE} 2>/dev/null || true",
        )
    ...
    cognito_user_deleted = auth._delete_channel_machine_user(tenant_id)
