...
# awscli v2 的 zip 按 CPU 架构分开发布(aarch64 / x86_64)。装错架构 → 二进制
# exec format error。按容器实际架构(uname -m)选,arm64 CodeBuild 得 aarch64、
# x86_64 CodeBuild 得 x86_64,同一个脚本两条环境都能跑。
AWSCLI_ARCH="$(uname -m)"  # aarch64 或 x86_64,正好是 awscli zip 的命名
echo "=== [container] 装 awscli v2 (${AWSCLI_ARCH}) ==="
curl -s "https://awscli.amazonaws.com/awscli-exe-linux-${AWSCLI_ARCH}.zip" -o /tmp/a.zip
