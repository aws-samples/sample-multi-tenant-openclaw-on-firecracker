...
def desired_cors_bundle(cors: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "integration": {
            "type": "MOCK",
            # 必须逐字复现 CDK 为本仓合成的 CORS 形状,而不是另写一份"更规范"的。判据:该
            # 栈里 65 个 OPTIONS 方法【全部】是 `{ statusCode: 200 }` 且【无一】显式设
            # passthroughBehavior。helper 的契约是"手工路径产出 CDK 会产出的东西",形状不一致
            # 就会让本工具建出的方法与 CDK 建的对不上,校验器的 CORS 身份判据因此恒不成立。
            # 行为等价:MOCK 集成的 request template 里那个 statusCode 是惰性的 —— 真正的
            # 204 来自下面的 integrationResponses;不设 passthroughBehavior 时 API Gateway
            # 的默认值就是 WHEN_NO_MATCH。所以对齐 CDK 是零行为代价。
...
def first_shape_difference(
    expected: Any, actual: Any, field: str = ""
) -> tuple[str, str, Any, Any] | None:
    if isinstance(expected, dict):
        if not isinstance(actual, dict):
            return ("drift", field or "<root>", expected, actual)
        for key in sorted(expected):
            child = f"{field}.{key}" if field else key
            if key not in actual:
                return ("unverified", child, expected[key], MISSING)
            difference = first_shape_difference(expected[key], actual[key], child)
            if difference is not None:
                return difference
        for key in sorted(actual):
            if key not in expected:
                child = f"{field}.{key}" if field else key
                return ("drift", child, MISSING, actual[key])
        return None
    if expected != actual:
        return ("drift", field or "<root>", expected, actual)
    return None
...
def shape_value(value: Any) -> str:
    return "<unavailable>" if value is MISSING else canonical(value)


def require_existing_a_shape(
    label: str,
    legacy_message: str,
    expected: dict[str, Any],
    actual: dict[str, Any],
    *,
    ...
    difference = first_shape_difference(expected, actual)
    if difference is None:
        if announce:
            print(f"already converged: {label}")
        return
    kind, field, wanted, live = difference
    if kind == "unverified":
        fail(
            f"{legacy_message}; existing target could not be verified at {field}: "
            f"expected={shape_value(wanted)} actual={shape_value(live)}"
        )
    fail(
        f"{legacy_message}; existing target drifted at {field}: "
        f"expected={shape_value(wanted)} actual={shape_value(live)}"
    )


def require_existing_a_resource(
    route: dict[str, Any],
    ...
    path = route["path"]
    parent = next(
        (item for item in all_resources if item.get("path") == parent_path(path)),
        None,
    )
    if parent is None or not parent.get("id"):
        fail(
            f"{path}: resource A requires an absent target; existing target "
            f"could not be verified at parentId because parent {parent_path(path)!r} "
            "has no readable id"
        )
    readable = {
        key: destination[key]
        for key in ("path", "parentId", "pathPart")
        if key in destination
    }
    require_existing_a_shape(
        f"resource {path}",
        f"{path}: resource A requires an absent target",
        {
            "path": path,
            "parentId": parent["id"],
            "pathPart": path_part(path),
        },
        readable,
        announce=False,
    )
    methods = destination.get("resourceMethods", {})
    if not isinstance(methods, dict):
        fail(
            f"{path}: resource A requires an absent target; existing target "
            "could not be verified at resourceMethods because the live value "
            "is not an object"
        )
    declared = {item["method"] for item in route["methods"]}
    if route["cors"]["change"] in {"A", "M"}:
        declared.add("OPTIONS")
    unexpected = sorted(set(methods) - declared)
    if unexpected:
        fail(
            f"{path}: resource A requires an absent target; existing target "
            f"drifted at resourceMethods: expected only {sorted(declared)} "
            f"actual={sorted(methods)}"
        )
    print(f"already converged: resource {path}")
...
def preflight(
    ...
    for route in spec["routes"]:
        ...
        if change == "A" and destination is not None:
            require_existing_a_resource(route, destination, all_resources)
        ...
        for method in route["methods"]:
            ...
            if method["change"] == "A" and before is not None:
                require_existing_a_shape(
                    f"method {method['method']} {lookup}",
                    f"{lookup}: method {method['method']} A already exists",
                    normalize_bundle(
                        desired_method_bundle(template, method["method"])
                    ),
                    normalize_bundle(before),
                )
                methods_already_converged[method["method"]] = True
        ...
        cors_already_converged = False
        if cors_change == "A" and cors_before is not None:
            require_existing_a_shape(
                f"CORS {lookup}",
                f"{lookup}: CORS A already exists",
                normalize_bundle(desired_cors_bundle(route["cors"])),
                normalize_bundle(cors_before),
            )
            cors_already_converged = True
        ...
        snapshots.append(
            {
                ...
                "methods_already_converged": methods_already_converged,
                "cors_before": cors_before,
                "cors_already_converged": cors_already_converged,
                "journal": {
                    "resource": "pending",
                    "methods": {
                        method["method"]: {
...
def apply_routes(
    ...
    for snapshot in state["routes"]:
        ...
        for method in route["methods"]:
            ...
            if (
                method["change"] == "A"
                and snapshot.get("methods_already_converged", {}).get(
                    method["method"]
                )
            ):
                method_journal["status"] = "completed"
            elif method["change"] in {"A", "M"}:
                gateway.put_bundle(
                    resource["id"],
                    method["method"],
                    desired_method_bundle(template, method["method"]),
        ...
        if cors_change == "A" and snapshot.get("cors_already_converged"):
