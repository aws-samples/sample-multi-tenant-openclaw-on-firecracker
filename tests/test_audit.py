...
class TestMutationAudits:
    def setup_method(self):
        ...
        # _ssm_send must return a truthy CommandId: since the 2026-07-01 SSM
        # throttle fix, _launch_vm returns _ssm_send's result and create_tenant
        # rolls back + 502s on a None (throttled) send. These audit tests
        # exercise the happy path, so the send "succeeds" with a fake id.
        self._patchers = [
            patch.object(api, "_ssm_run", return_value=True),
            patch.object(api, "_ssm_send", return_value="cmd-audit-ok"),
        ]
    ...
    def test_create_tenant_writes_audit(self):
        api.lambda_handler(
            _api_event("POST", "/tenants", body={"name": "x"}, api_key_id="key-1"), None
        )
    ...
    def test_delete_tenant_writes_audit(self):
        ...
        api.lambda_handler(
            _api_event("DELETE", "/tenants/{id}", path_params={"id": "t1"}), None
        )
        item = api.audit_table.put_item.call_args[1]["Item"]
    ...
    def test_put_template_writes_audit(self):
        ...
        api.lambda_handler(
            _api_event("PUT", "/templates/{name}", path_params={"name": "x"}), None
        )
...
class TestNoAuditOnReads:
    ...
    def test_get_tenant_not_audited(self):
        ...
        api.lambda_handler(
            _api_event("GET", "/tenants/{id}", path_params={"id": "t1"}), None
        )
        api.audit_table.put_item.assert_not_called()
...
class TestAuditFailureIsolation:
    ...
    def test_audit_failure_does_not_break_create(self):
        ...
        resp = api.lambda_handler(
            _api_event("POST", "/tenants", body={"name": "x"}), None
        )
...
class TestGetAuditLog:
    ...
    def test_since_filter_applied(self):
        ...
        assert "2026-05-01T00:00:00Z" in leaf_values, (
            f"expected since in {leaf_values!r}"
        )
