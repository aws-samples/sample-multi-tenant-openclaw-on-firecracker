...
- secrets 一律不进(vkey/cognito password host 侧从 DDB tenants 自取)。
...
# manifest line schema 允许字段(secrets 一律不进)。Guard 防以后有人往 manifest 里塞
# vkey/password 之类。
_ALLOWED_LINE_KEYS = frozenset({"t", "n", "c", "m", "e"})


def encode_manifest_line(tenant: Dict[str, Any]) -> str:
    """把一个租户装箱条目编码成一行 JSON。字段名短(t/n/c/m/e)压 part 数。
    ...
    # sort_keys=False:字段顺序固定 t/n/c/m/e(可预测,便于 diff/日志)
