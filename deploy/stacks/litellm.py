...
def build_litellm(self, ctx):
    ...
    if _aigw_url:
        ...
        _ha_lt = ec2.LaunchTemplate(
            ...
            instance_type=ec2.InstanceType(_aigw_cfg.get("instance_type", "c7i.large")),
        ...
        for _fn in filter(None, [api_fn, getattr(self, "_lifecycle_consumer", None)]):
            _fn.add_environment("LITELLM_MASTER_KEY_SECRET", litellm_secret.secret_name)
    ...
    if _prom_enabled:
        ...
        if metrics_cfg.get("grafana_alb", True):
            ...
            # self-hosted backend only.
            if api_fn is not None:
                api_fn.add_environment(
                    "SELF_HOSTED_GRAFANA_URL",
