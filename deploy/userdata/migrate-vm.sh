...
# #179 — restore 分支必须与 launch-vm.sh 建立 byte-identical 的 L4 网络隔离,否则
# 迁移复活的 VM 挂在一个无 iptables 规则的 tap 上,可跨租户串网 + 可达 IMDS(数据安全
# 类缺陷,#34/#41 一直漏的第二条「拉起 VM」路径)。SUBNET_PREFIX/EGRESS_* 从
...
_harden_restored_tap() {
  ...
  echo "  [#179] hardened restored tap ${tap} (${host_tap_ip}/30): IMDS+east-west+mgmt-port DROP"
}
...
case "$MODE" in
  ...
  restore)
    ...
    # #179 — L4 网络隔离必须在 /snapshot/load 之前建好:snapshot 按原 tap 名恢复
