...
> **2026-07-08 data-plane redesign notice**: the data-plane hub token exchange (`{HUB}/hub/token`, `/channel-token`, `/hub/ws`) and the Cognito data-plane identity described below have been **fully superseded** by [Chapter 13 · Data-plane two-tier routing](13-data-plane-redesign.md); the current data plane uses the gateway two-tier route, and Chapter 13 is authoritative for real-time chat integration. The control plane REST API (`{BASE}` endpoints) in this chapter remains valid.
...
> Verification sources: the control plane routes and RBAC levels come from the route table in `deploy/lambda/api/handler.py` (the `routes` dictionary) + the `_VIEWER_OK`/`_RBAC_SKIP`/`_rbac_check` definitions; endpoint behavior was verified with live `curl` against the deployment environment; the hub/wss parameters come from the legacy data-plane hub source (retired in the redesign; see Chapter 13 for the current data plane). The pagination page-size semantics and the AgentCore tool list follow the actual deployment configuration.
