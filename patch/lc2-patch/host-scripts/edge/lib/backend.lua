...
-- #606 — balancer 阶段不能用 cosocket；该提示把“重投后读 primary”的
-- read-after-write 要求交给下一次 rewrite 请求兑现。
local function retry_hint_key(tid) return "p:" .. tid end
...
-- #606 — balancer 重投只能读共享内存。复用 allow_stale=true 是为了在 fresh
-- 标记已过期时仍能看见别的 rewrite 刚换入的 peer；l2_get 会先判负缓存，
-- 因而权威删除后的旧 blob 绝不会被重投采用。
function _M.peek_cached(shared, tid)
    if not tid or tid == "" then return nil end
    local desc, is_neg = l2_get(shared, tid, true)
    if is_neg then return nil end
    return desc
end

-- #606 — 保留 L2 blob 作为 Redis 故障时的 fail-static 底料，只撤销本地和
-- fresh 标记，迫使下一次 rewrite 重读 Redis。negative marker 保持原样。
function _M.mark_retry_stale(shared, tid)
    if not tid or tid == "" then return end
    if L1_CACHE then L1_CACHE:delete(tid) end
    if shared then
        shared:delete(fresh_key(tid))
        shared:set(retry_hint_key(tid), "1", POS_TTL_SEC)
    end
end

-- #606 — 提示只消费一次，不加锁：并发请求里赢者读 primary 并回填缓存，
-- 输者按正常 reader 路径读取；两条路径都不会在 balancer 阶段触碰 Redis。
function _M.consume_retry_hint(shared, tid)
    if not shared or not tid or tid == "" then return false end
    if shared:get(retry_hint_key(tid)) == nil then return false end
    shared:delete(retry_hint_key(tid))
    return true
end
